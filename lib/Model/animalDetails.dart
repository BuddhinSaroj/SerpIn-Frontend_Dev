class AnimalDetails {
  String commonName;
  String scientificName;
  String venomousLevel;
  String description;
  String appearance;
  String geographicLocation;
  String lifeAndHabits;
  String sinhalaCommonName;
  String sinhalaScientificName;
  String sinhalaVenomousLevel;
  String sinhalaDescription;
  String sinhalaAppearance;
  String sinhalaGeographicLocation;
  String sinhalaLifeAndHabits;
  String imageUrl1;
  String imageUrl2;
  String imageUrl;

  AnimalDetails({
    required this.commonName,
    required this.scientificName,
    required this.venomousLevel,
    required this.description,
    required this.appearance,
    required this.geographicLocation,
    required this.lifeAndHabits,
    required this.sinhalaCommonName,
    required this.sinhalaScientificName,
    required this.sinhalaVenomousLevel,
    required this.sinhalaDescription,
    required this.sinhalaAppearance,
    required this.sinhalaGeographicLocation,
    required this.sinhalaLifeAndHabits,
    required this.imageUrl,
    required this.imageUrl1,
    required this.imageUrl2,
  });
}

List<AnimalDetails> snakeList = [
  AnimalDetails(
    commonName: 'Common Krait',
    scientificName: 'Bungarus Caeruleus',
    imageUrl: 'assets/snakes/common_krait.png',
    venomousLevel: 'Deadly Venomous',
    description:
        '- The Common krait (Bungarus caeruleus ) is a highly venomous snake native to the Indian subcontinent. It is a member of the "big four" species, inflicting the most snakebites on humans in Bangladesh and India.\nThe Common kraits venom consists mostly of powerful neurotoxin, which induce muscle paralysis. The estimated lethal dose for humans is 2-3 mg. Kraits are nocturnal, so seldom encounter humans during daylight; incidents occur mainly at night.\nCommon kraits are carnivores feeding primarily on other snakes, including: "blind worms" and other kraits, including the young. They also feed on small mammals, lizards, and frogs. The young are known to eat arthropods.',
    appearance:
        '- The coloration of this snake is generally black or bluish-black, with about 40 thin, white crossbars which may be indistinct or absent anteriorly. Albino specimens can be found, although such cases are extremely rare. The pattern, however, is complete and well-defined in the young, which are marked with conspicuous crossbars even anteriorly; in old individuals, the narrow white lines may be found as a series of connected spots, with a prominent spot on the vertebral region. A white particular spot may be present; the upper lips and the belly are white. Males of this species are typically longer, with proportionately longer tails.',
    geographicLocation:
        '- Common Kraits are distributed from Sindh to West Bengal, throughout South India and Sri Lanka. They have also been recorded in Afghanistan, Bangladesh, and Nepal. These snakes live in a wide variety of habitats, including fields and low scrub jungle, semi-desert, rocky areas, and oven ventures to plantations, cultivated land, suburban gardens, and settled areas. They rest in termite mounds, brick piles, rat holes, and even inside houses. Common kraits are frequently encountered in water or in proximity to a water source.',
    lifeAndHabits:
        '- Common kraits are solitary and can be active both during the day and night time. During the day, they are sluggish and generally docile. They often hide in rodent holes, loose soil, or beneath debris, so are rarely seen.',
    sinhalaCommonName: 'තෙල් කරවැලා/මගමරුවා',
    sinhalaScientificName: 'Bungarus Caeruleus',
    sinhalaVenomousLevel: 'මාරාන්තික විෂ සහිත',
    sinhalaDescription:
        'තෙල් කරවැලා (Bungarus caeruleus) යනු ඉන්දියානු උපමහාද්වීපයට ආවේණික උග්‍ර විෂ සහිත සර්පයෙකි. එය "Big Four" විශේෂයේ සාමාජිකයෙකි, බංග්ලාදේශයේ සහ ඉන්දියාවේ මිනිසුන්ට වැඩිපුරම සර්ප දෂ්ට කිරීම් සිදු කරයි.\nතෙල් කරවැලාගේ විෂ බොහෝ දුරට සමන්විත වන්නේ මාංශ පේශි අංශභාගය ඇති කරන ප්‍රබල නියුරොටොක්සින් ය. මිනිසුන් සඳහා ඇස්තමේන්තුගත මාරාන්තික මාත්‍රාව 2-3 mg වේ. තෙල් කරවැලා නිශාචර වේ, එබැවින් දිවා කාලයේ මිනිසුන් හමුවන්නේ කලාතුරකිනි. සිදුවීම් ප්‍රධාන වශයෙන් රාත්‍රියේ සිදු වේ.\nතෙල් කරවැලා යනු ප්‍රධාන වශයෙන් අනෙකුත් සර්පයන් මත පෝෂණය වන මාංශ භක්ෂකයින් වන අතර, ඒවා ඇතුළුව: "පණුවන්" සහ තෙල් කරවැල් පැටවුන් ඇතුළු අනෙකුත් කුඩා ක්ෂීරපායින්, කටුස්සන් සහ ගෙම්බන්ගෙන්ද පෝෂණය වෙයි.',
    sinhalaAppearance:
        '- මෙම සර්පයාගේ වර්ණය සාමාන්‍යයෙන් කළු හෝ නිල්-කළු වන අතර සිහින්, සුදු හරස් තීරු 40ක් පමණ ඇති අතර ඒවා ඉදිරිපස නොපැහැදිලි හෝ නොතිබිය හැකිය. එවැනි අවස්ථා අතිශයින් දුර්ලභ වුවද ඇල්බිනෝ නිදර්ශක සොයාගත හැකිය. කෙසේ වෙතත්, රටාව සම්පූර්ණ සහ මනාව අර්ථ දක්වා ඇති අතර, ඉදිරිපසින් පවා කැපී පෙනෙන හරස් තීරු වලින් සලකුණු කර ඇත; මහලු සර්පයන් තුළ පටු සුදු රේඛා සම්බන්ධිත ලප මාලාවක් ලෙස දක්නට ලැබේ, පෘෂ්ඨවංශික කලාපයේ ප්‍රමුඛ ස්ථානයක් ඇත. සුදු පැහැති පුර්ව ලපයක් තිබිය හැක, උඩුතොල් සහ උදරය සුදු ය. මෙම විශේෂයේ පිරිමි සාමාන්‍යයෙන් දිගු වන අතර සමානුපාතිකව දිගු වලිග ඇත.',
    sinhalaGeographicLocation:
        '- තෙල් කරවැලා සින්ද් සිට බටහිර බෙංගාලය දක්වා, දකුණු ඉන්දියාව සහ ශ්‍රී ලංකාව පුරා විසිරී පවතී. මොවුන් ඇෆ්ගනිස්ථානය, බංග්ලාදේශය සහ නේපාලයේ ද වාර්තා වී ඇත. මෙම සර්පයන් කෙත්වතු සහ පහත් ලඳු කැලෑ, අර්ධ කාන්තාර, පාෂාණ සහිත ප්‍රදේශ සහ වතු, වගා බිම්, තදාසන්න උද්‍යාන සහ ජනාකීර්ණ ප්‍රදේශවලට උඳුන් ඇතුළු විවිධ වාසස්ථානවල ජීවත් වේ. වේයන් ගොඩවල් වල, ගඩොල් ගොඩවල් වල, මී වලවල් වල, ගෙවල් ඇතුලේ පවා ඔවුන් විවේක ගන්නවා. තෙල් කරවැලා ජලයේ හෝ ජල ප්‍රභවයක් ආසන්නයේ නිතර දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- තෙල් කරවැලා හුදකලා වන අතර දිවා කාලයේ සහ රාත්‍රී කාලයේ ක්‍රියාකාරී විය හැක. දිවා කාලයේදී ඔවුන් මන්දගාමී වන අතර සාමාන්‍යයෙන් සාමකාමීය. ඔවුන් බොහෝ විට මීයන් සිදුරු, ලිහිල් පස් හෝ සුන්බුන් යට සැඟවී සිටින නිසා කලාතුරකින් දක්නට ලැබේ.',
    imageUrl1: 'assets/snakes/common_krait_2.png',
    imageUrl2: 'assets/snakes/common_krait_3.png',
  ),
  AnimalDetails(
    commonName: 'Russells Viper',
    scientificName: 'Daboia Russelii',
    imageUrl: 'assets/snakes/Russells_viper_1.jpg',
    venomousLevel: 'Deadly Venomous',
    description:
        '- Russells viper (Daboia russelii) is a venomous snake found in Asia. The species is named for Patrick Russell, a Scottish herpetologist who first described many of Indias snakes. In Bengali, this snake is called Chandroborha since it carries lenticular or more precisely lunar marks all over its body. Apart from being a member of the big four snakes in India, Russells viper is also responsible for causing the most snakebite incidents and deaths among all venomous snakes.',
    appearance:
        '- These snakes are deep yellow, tan, or brown in color, with three series of dark brown spots that run the length of the body. Each of these spots has a black ring around it, the outer border of which is intensified with a rim of white or yellow. The head has a pair of distinct dark patches, one on each temple, together with a pinkish, salmon, or brownish V or X marking that forms an apex towards the snout. Behind the eye is a dark streak, outlined in white, pink, or buff. The venter is white, whitish, yellowish, or pinkish, often with an irregular scattering of dark spots.',
    geographicLocation:
        '- Russells vipers are found in India, Sri Lanka, Bangladesh, Nepal, Myanmar, Thailand, Pakistan, Cambodia, Tibet, China (Guangxi, Guangdong), Taiwan, and Indonesia (Endeh, Flores, east Java, Komodo, and Lomblen Islands). These snakes are not restricted to any particular habitat but do tend to avoid dense forests. They are mostly found in open, grassy, or bushy areas, but may also be found in second-growth forests (scrub jungles), on forested plantations, and on farmland. Russells vipers are most common in plains, coastal lowlands, and hills of suitable habitat. Humid environments, such as marshes, swamps, and rainforests, are avoided. They are also often found in highly urbanized areas.',
    lifeAndHabits:
        '- Russells vipers are solitary terrestrial creatures. They are primarily nocturnal foragers. However, during cool weather, they become more active during the day. These snakes like to bask in the sun during the day. The rest time is spent hiding in the caves, in cracks in the soil, or under the leaf litter. Adults move slowly and sluggishly unless they are threatened; after that, they can become very aggressive. Russells vipers form a series of S-loops, raise the first third of the body, and produce a hiss that is supposedly louder than that of any other snake. When striking from this position, they can exert so much force that even a large individual can lift most of their body off the ground in the process. These snakes are strong and may react violently to being picked up. The bite may be a snap, or they may hang on for many seconds.',
    sinhalaCommonName: 'තිත් පොළඟා/දාර පොළඟා',
    sinhalaScientificName: 'Daboia Russelii',
    sinhalaVenomousLevel: 'මාරාන්තික විෂ සහිත',
    sinhalaDescription:
        'තිත් පොළඟා (Daboia Russelii) යනු ආසියාවේ දක්නට ලැබෙන විෂ සහිත සර්පයෙකි. ඉන්දියාවේ සර්පයින් බොහොමයක් මුලින්ම විස්තර කළ ස්කොට්ලන්ත හර්පෙටොලොජිස්ට්වරයෙකු වන පැට්‍රික් රසල් වෙනුවෙන් මෙම විශේෂය නම් කර ඇත. බෙංගාලි භාෂාවෙන්, මෙම සර්පයා චන්ද්‍රොබෝර්හා ලෙස හඳුන්වනු ලබන්නේ උගේ ශරීරය පුරා කාච හෝ වඩාත් නිවැරදිව චන්ද්‍ර සලකුණු පවතින බැවිනි. සියලුම විෂ සහිත සර්පයන් අතර වැඩිම සර්ප දෂ්ට කිරීම් සහ මරණ සිදු කිරීමට ද තිත් පොළඟා වගකිව යුතුය.',
    sinhalaAppearance:
        '- මෙම සර්පයන් ගැඹුරු කහ, දුඹුරු හෝ දුඹුරු පැහැයෙන් යුක්ත වන අතර සිරුරේ දිගට දිවෙන තද දුඹුරු පැහැ ලප ශ්‍රේණි තුනක් ඇත. මෙම සෑම ලපයක්ම වටා කළු වළල්ලක් ඇති අතර, එහි පිටත මායිම සුදු හෝ කහ පැහැති දාරයක් සමඟ තීව්‍ර වේ. හිසෙහි එකිනෙකට වෙනස් අඳුරු පැල්ලම් යුගලයක් ඇත, එක් එක් දේවාලය මත එකක්, රෝස පැහැති, සැමන් හෝ දුඹුරු පැහැති V හෝ X සලකුණක් සමඟින් නාසය දෙසට අග්‍රයක් සාදයි. ඇසට පිටුපසින් සුදු, රෝස ලෙස දක්වා ඇති අඳුරු ඉරි ඇත. බොහෝ විට අක්‍රමවත් ලෙස අඳුරු පැල්ලම් විසිරී යයි.',
    sinhalaGeographicLocation:
        '- තිත් පොළඟා ඉන්දියාව, ශ්‍රී ලංකාව, බංග්ලාදේශය, නේපාලය, මියන්මාරය, තායිලන්තය, පකිස්ථානය, කාම්බෝජය, ටිබෙටය, චීනය (ගුවැන්සි, ගුවැන්ඩොං), තායිවානය සහ ඉන්දුනීසියාව (එන්ඩේ, ෆ්ලෝරස්, නැගෙනහිර ජාවා, කොමෝඩෝ සහ ලොම්බලන් දූපත්) යන රටවල දක්නට ලැබේ. මෙම සර්පයන් කිසියම් විශේෂ වාසස්ථානයකට සීමා නොවන නමුත් ඝන වනාන්තර මග හැරීමට නැඹුරු වෙති. තිත් පොළඟා බොහෝ දුරට විවෘත, තෘණ හෝ පඳුරු සහිත ප්‍රදේශවල දක්නට ලැබේ, නමුත් දෙවන වර්ධනය වන වනාන්තරවල (ලඳු කැලෑවල), වනාන්තර වැවිලිවල සහ ගොවිබිම් වලද දක්නට ලැබේ. තිත් පොළඟා වඩාත් සුලභ වන්නේ තැනිතලා, වෙරළබඩ පහත් බිම් සහ සුදුසු වාසස්ථාන සහිත කඳුකරයේ ය. වගුරු බිම්, වගුරු බිම් සහ වැසි වනාන්තර වැනි තෙත් පරිසරයන් වැළකෙයි. තිත් පොළඟා බොහෝ විට නාගරීකරණය වූ ප්‍රදේශවල ද දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- තිත් පොළඟා යනු හුදකලා භූමිෂ්ඨ ජීවීන් වේ. ඔවුන් මූලික වශයෙන් නිශාචර ආහාර සොයන්නන් වේ. කෙසේ වෙතත්, සිසිල් කාලගුණය තුළ, ඔවුන් දිවා කාලයේ වඩාත් ක්රියාකාරී වේ. මෙම සර්පයන් දිවා කාලයේ හිරු රශ්මියෙන් බැසීමට කැමතියි. විවේක කාලය ගත කරන්නේ ගුහාවල, පසෙහි ඉරිතැලීම්වල හෝ කොළ රොඩු යට සැඟවී සිටීමයි. වැඩිහිටි සර්පයන්ට තර්ජනයක් නොමැති නම් සෙමින් හා මන්දගාමීව ගමන් කරයි. ඊට පසු, ඔවුන් ඉතා ආක්රමණශීලී විය හැකිය. තිත් පොළඟා S-ලූප මාලාවක් සාදයි, ශරීරයේ පළමු තුනෙන් කොටස ඉහළට ඔසවා, වෙනත් ඕනෑම සර්පයෙකුගේ ශබ්දයට වඩා ශබ්දයක් ඇති කරයි. මෙම ඉරියව්වෙන් පහර දෙන විට, ඔවුන්ට කොතරම් බලයක් යෙදිය හැකිද යත්, විශාල පුද්ගලයෙකුට පවා ඔවුන්ගේ ශරීරයෙන් වැඩි ප්‍රමාණයක් ක්‍රියාවලියේදී බිමෙන් එසවිය හැකිය. මෙම සර්පයන් ශක්තිමත් වන අතර අල්ලා ගැනීමේදී දැඩි ලෙස ප්‍රතිචාර දැක්විය හැක. කටගැස්ම ක්ෂණික විය හැකිය, නැතහොත් ඒවා තත්පර ගණනාවක් එල්ලා තැබිය හැකිය.',
    imageUrl1: 'assets/snakes/Russells_viper_2.jpg',
    imageUrl2: 'assets/snakes/Russells_viper_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Saw-Scaled Viper',
    scientificName: 'Echis Carinatus',
    imageUrl: 'assets/snakes/saw_scaled_viper_3.jpg',
    venomousLevel: 'Deadly Venomous',
    description:
        '- Echis carinatus is a venomous viper species found in parts of the Middle East and Central Asia, and especially the Indian subcontinent. It is the smallest member of the big four snakes that are responsible for causing the most snakebite cases and deaths, due to various factors including their frequent occurrence in highly populated regions, and their inconspicuous nature. Two subspecies are currently recognized, including the nominate subspecies described here.',
    appearance:
        '- Saw-scaled viper is a venomous snake native to Asia. It is the smallest member of the big four snakes that are responsible for causing the most snakebite cases and deaths. Saw-scaled vipers have a pale buff, grayish, reddish, olive, or pale brown ground color, overlaid middorsal with a series of variably colored, but mostly whitish spots, edged with dark brown, and separated by lighter interblotch patches. The top of the head has a whitish cruciform or trident pattern and there is a faint stripe running from the eye to the angle of the jaw. Their belly is whitish to pinkish, uniform in color, or with brown dots that are either faint or distinct.',
    geographicLocation:
        '- Saw-scaled vipers are found in parts of the Middle East and Central Asia, and especially the Indian subcontinent. On the Indian subcontinent, they are found in India, Sri Lanka, Bangladesh, and Pakistan (including Urak near Quetta and Astola Island off the coast of Makran).',
    lifeAndHabits:
        '- These snakes are mostly crepuscular and nocturnal, although they have also been seen during daylight hours. During the daytime, they usually hide in deep mammal burrows, rock fissures, and fallen rotted logs. In sandy environments, they may bury themselves leaving only the head exposed. Often, they are most active after rains or on humid nights. Indian saw-scaled vipers often climb in bushes and shrubs, sometimes as much as 2 m above the ground. Their characteristic pose, a double coil with a figure of eight, with the head poised in the center, permits them to lash out like a released spring.',
    sinhalaCommonName: 'වැලි පොළඟා',
    sinhalaScientificName: 'Echis Carinatus',
    sinhalaVenomousLevel: 'මාරාන්තික විෂ සහිත',
    sinhalaDescription:
        'වැලි පොළඟා යනු මැද පෙරදිග සහ මධ්‍යම ආසියාවේ, දකුණු ආසියාවෙ සහ විශේෂයෙන් ඉන්දියානු උප මහද්වීපයේ සමහර ප්‍රදේශවල දක්නට ලැබෙන විෂ සහිත පොළඟ විශේෂයකි. ඔවුන්ගේ නොපෙනෙන ස්වභාවය ඇතුළු විවිධ සාධක හේතුවෙන් වැඩිම සර්ප දෂ්ට කිරීම් සහ මරණ සිදු කිරීමට වගකිව යුතු Big Four Snakes හි කුඩාම සාමාජිකයා මෙයයි. මෙහි විස්තර කර ඇති නාමයෝජනා උප විශේෂ ඇතුළුව උප විශේෂ දෙකක් දැනට හඳුනාගෙන ඇත.',
    sinhalaAppearance:
        '- වැලි පොළඟා යනු ආසියාවට ආවේණික විෂ සහිත සර්පයෙකි. වැඩිම සර්ප දෂ්ට කිරීම් සහ මරණ සිදු කිරීමට වගකිව යුතු විශාල සර්පයන් හතර දෙනාගෙන් කුඩාම සාමාජිකයා මෙයයි. වැලි පොළඟා හට සුදුමැලි බුෆ්, අළු, රතු, ඔලිව්, හෝ සුදුමැලි දුඹුරු බිම් වර්ණය ඇත, විවිධ වර්ණ සහිත, නමුත් බොහෝ දුරට සුදු පැහැති ලප මාලාවක් සහිත, තද දුඹුරු දාර සහිත, සහ සැහැල්ලු අන්තර් බ්ලොච් පැල්ලම් වලින් වෙන් කරන ලද මැද හිස මුදුනේ සුදු පැහැති කුරුස ආකෘතියක් හෝ ත්‍රිශූල රටාවක් ඇති අතර ඇසේ සිට හකු කෝණය දක්වා දිවෙන දුර්වල තීරුවක් ඇත. ඔවුන්ගේ බඩ සුදු පැහැයේ සිට රෝස පැහැයක් ගනී, ඒකාකාරී වර්ණයෙන් හෝ දුඹුරු පැහැති තිත් වලින් යුක්ත වන අතර ඒවා ක්ලාන්ත හෝ වෙනස් වේ.',
    sinhalaGeographicLocation:
        '- තවැලි පොළඟා මැදපෙරදිග සහ මධ්‍යම ආසියාවේ සමහර ප්‍රදේශවල සහ විශේෂයෙන් ඉන්දියානු උපමහාද්වීපයේ දක්නට ලැබේ. ඔවුන් ඉන්දියාව, ශ්‍රී ලංකාව, බංග්ලාදේශය සහ පකිස්ථානයේ (ක්වෙටා අසල යුරක් සහ මැක්‍රාන් වෙරළට ඔබ්බෙන් වූ ඇස්ටෝලා දූපත ඇතුළුව) දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- මෙම සර්පයන් බොහෝ විට නිශාචර වේ, නමුත් ඔවුන් දිවා කාලයේ ද දක්නට ඇත. දිවා කාලයේදී, ඔවුන් සාමාන්‍යයෙන් ගැඹුරු වලවල්, පාෂාණ ඉරිතැලීම් සහ වැටුණු කුණු වූ ලොගවල සැඟවී සිටිති. වැලි සහිත පරිසරයක, ඔවුන් තම හිස පමණක් නිරාවරණය වන පරිදි ඇත. බොහෝ විට, ඔවුන් වඩාත් ක්රියාකාරී වන්නේ වර්ෂාවෙන් පසුව හෝ තෙත් රාත්රීවලදීය. බොහෝ විට පඳුරු සහ පඳුරු මත, සමහර විට බිම සිට මීටර් 2 ක් දක්වා ඉහළ යයි. ඔවුන්ගේ ලාක්ෂණික ඉරියව්ව, අටක රූපයක් සහිත ද්විත්ව දඟරයක්, හිස මධ්‍යයේ තබා ඇති අතර, මුදා හරින ලද spring මෙන් ඔවුන්ට පහර දීමට ඉඩ සලසයි.',
    imageUrl1: 'assets/snakes/saw_scaled_viper_1.jpg',
    imageUrl2: 'assets/snakes/saw_scaled_viper_2.jpg',
  ),
  AnimalDetails(
    commonName: 'Hump Nosed Viper',
    scientificName: 'Hypnale Hypnale',
    imageUrl: 'assets/snakes/hypnale_hypnale_1.png',
    venomousLevel: 'Venomous',
    description:
        '- Hypnale hypnale is a venomous pit viper species found in Sri Lanka and India. Based on the taxonomic revisions done so far, have stated that there are four species that belong to the genus Hypnale which live in Sri Lanka (Including possibly new species Hypnale sp. ‘amal’). Among them, population of H. hypnale in Sri Lanka has been observed mainly in anthropogenic habitats and no observations have been made in undisturbed forests.At a glance, morphologically all these four species look similar. But with a closer look, H. hypnale can be easily distinguished from its congeners using scalation and with the help of non raised snout tip. This species is more active at night and in the day time they live under the logs, rocks and in the leaf litter. Venom of this genus mainly causes local envenoming, coagulopathy, acute renal failure and death.',
    appearance:
        '- The color pattern is grayish with heavy brown mottling, overlaid with a double row of large dark spots. The belly is brownish or yellowish with dark mottling. The tip of the tail is yellow or reddish.',
    geographicLocation:
        '- Hypnale hypnale is found in peninsular India to the Western Ghats as far north as 16° N, and in Sri Lanka. Hypnale hypnale is found in dense jungle and coffee plantations in hilly areas.',
    lifeAndHabits:
        '- Hypnale hypnale is active during early morning and night. It spends the day in leaf litter and thick bushes. This species can be found on the stream side basking during the sunrise. Although it is a slow mover, it is capable of fast strikes. It has an irritable disposition and will vibrate its tail when annoyed, a behavior it has in common with other pit vipers, especially rattlesnakes of the genera Crotalus and Sistrurus. It has been described as nocturnal, terrestrial, and aggressive when disturbed. It is the snake to cause the highest number of recorded snake bites in Sri Lanka.',
    sinhalaCommonName: 'පොලොන් තෙලිස්සා/කුණක‍ටුවා',
    sinhalaScientificName: 'Hypnale Hypnale',
    sinhalaVenomousLevel: 'විෂ සහිත',
    sinhalaDescription:
        'පොලොන් තෙලිස්සා යනු ශ්‍රී ලංකාවේ සහ ඉන්දියාවේ දක්නට ලැබෙන විෂ සහිත පොලග විශේෂයකි. මෙතෙක් සිදු කර ඇති වර්ගීකරණ සංශෝධන මත පදනම්ව, ශ්‍රී ලංකාවේ ජීවත් වන Hypnale ගණයට අයත් විශේෂ හතරක් ඇති බව ප්‍රකාශ කර ඇත. ඒ අතරින් ශ්‍රී ලංකාවේ H. ගනයේ ප්‍රධාන වශයෙන් මානව වාසස්ථානවල නිරීක්ෂණය වී ඇති අතර නොකැළඹුණු වනාන්තරවල නිරීක්ෂණ සිදු කර නොමැත. බැලූ බැල්මට රූප විද්‍යාත්මකව මෙම විශේෂ හතරම සමාන බව පෙනේ. නමුත් සමීපව බැලීමෙන්, H. hypnale එහි සංඝටක වලින් විශාලනය භාවිතයෙන් සහ උස් නොකෙරෙන තුණ්ඩය ආධාරයෙන් පහසුවෙන් වෙන්කර හඳුනාගත හැකිය. මෙම විශේෂය රාත්‍රියේදී වඩාත් ක්‍රියාකාරී වන අතර දිවා කාලයේදී ඔවුන් ජීවත් වන්නේ දැව, ගල් සහ කොළ රොඩු වල ය. මෙම කුලයට අයත් විෂ ප්‍රධාන වශයෙන් දේශීය විෂ වීම, කැටි ගැසීම්, උග්‍ර වකුගඩු අකර්මණ්‍ය වීම සහ මරණයට හේතු වේ.',
    sinhalaAppearance:
        '- මෙම සර්පයා විශාල අඳුරු ලප ද්විත්ව පේළියකින් ආවරණය කර ඇති තද දුඹුරු පැහැ ලප සහිත වර්ණ රටාව අළු පැහැයක් ගනී. උදරය දුඹුරු හෝ කහ පැහැයට හුරු අඳුරු පැල්ලම් සහිත වේ. වලිගයේ කෙළවර කහ හෝ රතු පාටයි.',
    sinhalaGeographicLocation:
        '- පොලොන් තෙලිස්සා ඉන්දියාවේ අර්ධද්වීපයේ සිට බටහිර ඝාට්ස් දක්වා උතුරට 16° N දක්වා සහ ශ්‍රී ලංකාවේ දක්නට ලැබේ. පොලොන් තෙලිස්සා ඝන කැලෑවල සහ කඳුකර ප්‍රදේශවල කෝපි වතුවල දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- පොලොන් තෙලිස්සා උදෑසන සහ රාත්‍රියේ සක්‍රීය වේ. පොලොන් තෙලිස්සා දවස ගත කරන්නේ කොළ රොඩු සහ ඝන පඳුරු වලය. මෙම විශේෂය හිරු නැඟෙන විට දිය පහර දෙපස දක්නට ලැබේ. සෙමින් චලනය වන නමුත්, වේගයෙන් පහර දීමට සමත් වේ. කෝපයට පත් වන ස්වභාවයක් ඇති අතර කෝපයට පත් වූ විට උගේ වලිගය කම්පනය කරයි, එය අනෙකුත් වයිපර්, විශේෂයෙන් ක්‍රොටලස් සහ සිස්ට්‍රුරස් ගණයේ රැට්ල්ස්නේක් සමඟ සාමාන්‍ය හැසිරීමකි. පොලොන් තෙලිස්සා නිශාචර, භූමිෂ්ඨ සහ කලබල වූ විට ආක්‍රමණශීලී ලෙස විස්තර කර ඇත. ශ්‍රී ලංකාවේ වැඩිම සර්ප දෂ්ට කිරීම් සංඛ්‍යාවක් වාර්තා කළ සර්පයා මෙයයි.',
    imageUrl1: 'assets/snakes/hypnale_hypnale_2.png',
    imageUrl2: 'assets/snakes/hypnale_hypnale_3.png',
  ),
  AnimalDetails(
    commonName: 'Corbra',
    scientificName: 'Naja naja',
    imageUrl: 'assets/snakes/cobra_1.png',
    venomousLevel: 'Deadly Venomous',
    description:
        '- The cobra found in Sri Lanka is known as the Sri Lankan cobra or Naja naja. This venomous snake is one of the most common and dangerous species of snakes found in Sri Lanka. It has a distinctive appearance, with a hooded head and a sleek, cylindrical body that can grow up to 2 meters in length. Sri Lankan cobras come in a variety of colors, from brown to black, and can be identified by their distinct patterns of scales on their skin. These cobras are known for their highly toxic venom, which can cause paralysis, respiratory failure, and death if left untreated. Despite their dangerous reputation, Sri Lankan cobras are an important part of the ecosystem and play a crucial role in controlling the rodent population.',
    appearance:
        '- These snakes vary tremendously in color and pattern throughout their range. The ventral scales or the underside coloration of this species can be grey, yellow, tan, brown, reddish or black. Dorsal scales may have a hood mark or color patterns. Salt-and-pepper speckles, especially in adult specimens, are seen on the dorsal scales. Cobras can easily be identified by their relatively large and quite impressive hood, which they expand when threatened. Many specimens exhibit a hood mark. This hood mark is located at the rear of the Cobras hood. When the hood mark is present, are two circular ocelli patterns connected by a curved line, evoking the image of spectacles.',
    geographicLocation:
        '- cobras are native to the Indian subcontinent and can be found throughout India, Pakistan, Sri Lanka, Bangladesh, and southern Nepal. Cobras inhabit a wide range of habitats throughout their geographical range. They can be found in dense or open forests, plains, agricultural lands (rice paddy fields, wheat crops), rocky terrain, wetlands, and they can even be found in heavily populated urban areas, such as villages and city outskirts. Cobras are often found in the vicinity of water.',
    lifeAndHabits:
        '- Most cobras are generally solitary and diurnal creatures. Indian cobras find their shelter in holes in embankments, tree hollows, termite mounds, rock piles, and small mammal dens. When threatened, these snakes take the characteristic posture they are famous for. They raise the front of the body and expand their distinctive hood, on which are resembled eyes-like hood marks. Indian cobras are one of the big four snakes of South Asia which are responsible for the majority of human deaths by snakebite in Asia.',
    sinhalaCommonName: 'නාගයා',
    sinhalaScientificName: 'Naja naja',
    sinhalaVenomousLevel: 'මාරාන්තික විෂ සහිත',
    sinhalaDescription:
        'ශ්‍රී ලංකාවෙන් හමුවන නාගයා ශ්‍රී ලංකා නාගයා හෙවත් Naja naja ලෙස හැඳින්වේ. මෙම විෂ සහිත සර්පයා ශ්‍රී ලංකාවේ බහුලව දක්නට ලැබෙන සහ භයානක සර්ප විශේෂයකි. එය කැපී පෙනෙන පෙනුමක් ඇති අතර, හිස් වැසුම් සහිත හිසක් සහ දිග මීටර් 2 ක් දක්වා වර්ධනය විය හැකි සිනිඳු, සිලින්ඩරාකාර සිරුරක් ඇත. ශ්‍රී ලංකාවේ නාගයන් දුඹුරු සිට කළු දක්වා විවිධ වර්ණවලින් යුක්ත වන අතර, ඔවුන්ගේ සමෙහි ඇති කොරපොතුවල වෙනස් රටා මගින් හඳුනාගත හැකිය. මෙම නාගයන් උග්‍ර විෂ සහිත විෂ සඳහා ප්‍රසිද්ධ වී ඇති අතර, ප්‍රතිකාර නොකළහොත් අංශභාගය, ශ්වසන අපහසුතා සහ මරණයට හේතු විය හැක. ඔවුන්ගේ භයානක කීර්තිය තිබියදීත්, ශ්‍රී ලංකාවේ නාගයන් පරිසර පද්ධතියේ වැදගත් කොටසක් වන අතර මීයන් ගහනය පාලනය කිරීමේදී තීරණාත්මක කාර්යභාරයක් ඉටු කරයි.',
    sinhalaAppearance:
        '- මෙම සර්පයන් ඔවුන්ගේ පරාසය පුරාවටම වර්ණයෙන් හා රටාවෙන් විශාල ලෙස වෙනස් වේ. මෙම විශේෂයේ උදර කොරපොතු හෝ යටි වර්ණය අළු, කහ, දුඹුරු, දුඹුරු, රතු හෝ කළු විය හැකිය. පෘෂ්ඨීය කොරපොතු මත දක්නට ලැබේ. තර්ජන එල්ල වූ විට ප්‍රසාරණය වන සාපේක්ෂ විශාල හා තරමක් ආකර්ෂණීය පෙනය මගින් නාගයන් පහසුවෙන් හඳුනාගත හැකිය. බොහෝ නිදර්ශක පෙනය පෙන්වයි. මෙම හුඩ් ලකුණ පිටුපස පිහිටා ඇත.',
    sinhalaGeographicLocation:
        '- නාගයන් ඉන්දියානු උපමහාද්වීපයට ආවේණික වන අතර ඉන්දියාව, පකිස්ථානය, ශ්‍රී ලංකාව, බංග්ලාදේශය සහ දකුණු නේපාලය පුරා දක්නට ලැබේ. නාගයන් ඔවුන්ගේ භූගෝලීය පරාසය පුරා පුළුල් පරාසයක වාසස්ථානවල වාසය කරයි. ඔවුන් ඝන හෝ විවෘත වනාන්තර, තැනිතලා, කෘෂිකාර්මික ඉඩම් (සහල් කුඹුරු, තිරිඟු භෝග), පාෂාණමය භූමි, තෙත් බිම්වල සොයා ගත හැකි අතර, ගම් සහ නගර මායිම් වැනි අධික ජනාකීර්ණ නාගරික ප්රදේශ වල පවා සොයාගත හැකිය. නාගයන් බොහෝ විට ජලය ආසන්නයේ දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- බොහෝ නාගයන් සාමාන්‍යයෙන් හුදකලා සහ දෛනික ජීවීන් වේ. නාගයන් ඔවුන්ගේ නවාතැන් සොයා ගන්නේ බැමි, ගස් කුහර, වේයන් ගොඩවල්, ගල් ගොඩවල් සහ කුඩා ක්ෂීරපායී ගුහා වල සිදුරු වලිනි. තර්ජනයට ලක් වූ විට, මෙම සර්පයන් ඔවුන් ප්‍රසිද්ධ වූ ලාක්ෂණික ඉරියව්ව ගනී. ඔවුන් ශරීරයේ ඉදිරිපස ඉහළට ඔසවන අතර ඔවුන්ගේ සුවිශේෂී පෙනය පුළුල් කරයි, ඒවා මත ඇස් වැනි සලකුණු වලට සමාන වේ. නාගයන් දකුණු ආසියාවේ විශාල සර්පයන් හතර දෙනාගෙන් එකකි, ඔවුන් ආසියාවේ සර්ප දෂ්ට කිරීම් වලින් බහුතරයක් මිනිස් මරණවලට වගකිව යුතුය.',
    imageUrl1: 'assets/snakes/cobra_2.png',
    imageUrl2: 'assets/snakes/cobra_3.png',
  ),
  AnimalDetails(
    commonName: 'Green Vine Snake',
    scientificName: 'Ahaetulla nasuta',
    imageUrl: 'assets/snakes/ahaetulla_3.png',
    venomousLevel: 'Non-venomous',
    description:
        '- Ahaetulla nasuta, also known as the Sri Lankan green vine snake, is a non-venomous species of snake that can be found in Sri Lanka. It is a slender and agile snake, with a long and thin body that can grow up to 1.5 meters in length. The Sri Lankan green vine snake has a bright green coloration, with a white or yellow underside, and is characterized by its pointed snout and large eyes, which give it excellent vision.',
    appearance:
        '- This species of snake has a long and slender body, which can grow up to 1.5 meters in length. It is green in color, with a bright and vibrant hue that can vary in intensity. The underside of the snake is usually white or yellow.\nThe Sri Lankan green vine snake has a pointed snout and large eyes, which give it excellent vision. Its eyes are forward-facing, providing binocular vision that helps it to accurately judge distance and accurately strike at prey. The scales on its body are smooth and overlapping, giving it a sleek appearance and allowing it to move easily through vegetation.\nOne of the most interesting aspects of the Sri Lankan green vine snakes appearance is its ability to change color. When threatened or disturbed, it may change its green coloration to brown or yellow in order to blend in with its surroundings and avoid detection. This makes it a master of camouflage and a difficult predator to spot in the wild.',
    geographicLocation:
        '- Ahaetulla nasuta, the Sri Lankan green vine snake, is a native species of Sri Lanka. It can be found throughout the island, from the lowland rainforests to the montane forests in the central highlands. Sri Lanka is an island nation located off the southern coast of India, in the Indian Ocean. The country is home to a wide variety of flora and fauna, much of which is endemic to the island, including several species of snakes such as the Sri Lankan green vine snake.',
    lifeAndHabits:
        '- This species is primarily active during the day and is most commonly seen basking in the sun on branches or leaves. The Sri Lankan green vine snake is a skilled climber and uses its long and slender body to move easily through the branches. The diet of the Sri Lankan green vine snake consists mainly of small animals such as lizards, frogs, and insects. It uses its excellent eyesight to locate prey and then strikes quickly to capture it. This species is known for its unique hunting technique, in which it will approach its prey slowly and then make a sudden lunge to catch it off guard. Sri Lankan green vine snakes are generally solitary animals and are most commonly seen alone.',
    sinhalaCommonName: 'ඇහැටුල්ලා / ඇස්ගුල්ලා',
    sinhalaScientificName: 'Ahaetulla nasuta',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        'ශ්‍රී ලංකාවේ කොළ වැල් සර්පයා ලෙසින්ද හඳුන්වනු ලබන Ahaetulla nasuta යනු ශ්‍රී ලංකාවේ දක්නට ලැබෙන විෂ සහිත නොවන සර්ප විශේෂයකි. එය දිග මීටර් 1.5 ක් දක්වා වර්ධනය විය හැකි දිගු හා සිහින් සිරුරක් සහිත සිහින් සහ කඩිසර සර්පයෙකි. ශ්‍රී ලංකාවේ කොළ පැහැති වැල් සර්පයා සුදු හෝ කහ යටි පැත්තක් සහිත දීප්තිමත් කොළ පැහැයකින් යුක්ත වන අතර උගේ උල් වූ නහය සහ විශාල ඇස් වලින් සංලක්ෂිත වන අතර එමඟින් ඔහුට විශිෂ්ට දර්ශනයක් ලබා දේ.',
    sinhalaAppearance:
        '- මෙම සර්ප විශේෂය දිගු හා සිහින් සිරුරක් ඇති අතර, දිග මීටර් 1.5 ක් දක්වා වර්ධනය විය හැකිය. එය තීව්රතාවයෙන් වෙනස් විය හැකි දීප්තිමත් හා විචිත්රවත් පැහැයක් සහිත හරිත වර්ණයෙන් යුක්ත වේ. සර්පයාගේ යටි පැත්ත සාමාන්‍යයෙන් සුදු හෝ කහ පැහැයක් ගනී.\nශ්‍රී ලංකාවේ කොළ පැහැති වැල් සර්පයාට උල් වූ නාසයක් සහ විශාල ඇස් ඇති අතර එමඟින් ඔහුට විශිෂ්ට පෙනීම ලබා දේ. උගේ ඇස් ඉදිරි දෙසට යොමු වී ඇති අතර, දුර නිවැරදිව විනිශ්චය කිරීමට සහ ගොදුරට නිවැරදිව පහර දීමට උපකාරී වන දුරදක්න දර්ශනය සපයයි. උගේ සිරුරේ ඇති කොරපොතු සිනිඳු සහ අතිච්ඡාදනය වන අතර, එය සිනිඳු පෙනුමක් ලබා දෙන අතර වෘක්ෂලතා හරහා පහසුවෙන් ගමන් කිරීමට ඉඩ සලසයි.\nශ්‍රී ලංකාවේ කොළ වැල් සර්පයාගේ පෙනුමේ වඩාත් සිත්ගන්නා කරුණක් වන්නේ වර්ණය වෙනස් කිරීමට ඇති හැකියාවයි. තර්ජනයට ලක් වූ විට හෝ බාධා වූ විට, එහි වටපිටාව සමඟ මිශ්‍ර වීමට සහ හඳුනා ගැනීම වළක්වා ගැනීම සඳහා එහි කොළ පැහැය දුඹුරු හෝ කහ පැහැයට වෙනස් කළ හැකිය. මෙය සැඟවීමේ ප්‍රවීණයෙකු වන අතර වනයේ හඳුනා ගැනීමට අපහසු විලෝපිකයෙකු බවට පත් කරයි.',
    sinhalaGeographicLocation:
        '- Ahaetulla nasuta, ශ්‍රී ලංකාවේ කොළ වැල් සර්පයා, ශ්‍රී ලංකාවේ දේශීය විශේෂයකි. පහතරට වැසි වනාන්තරවල සිට මධ්‍යම කඳුකරයේ කඳුකර වනාන්තර දක්වා දිවයින පුරා දක්නට ලැබේ. ශ්‍රී ලංකාවේ කොළ වැල් සර්පයා වැනි සර්ප විශේෂ කිහිපයක් ඇතුළුව දිවයිනට ආවේණික වන බොහෝ ශාක හා සත්ත්ව විශේෂවලට නිවහන වේ.',
    sinhalaLifeAndHabits:
        '- මෙම විශේෂය ප්‍රධාන වශයෙන් දිවා කාලයේ ක්‍රියාකාරී වන අතර අතු හෝ පත්‍ර මත හිරු රශ්මියෙන් බැසීම බහුලව දක්නට ලැබේ. ශ්‍රී ලංකාවේ හරිත වැල් සර්පයා දක්ෂ කඳු නගින්නෙකු වන අතර අතු හරහා පහසුවෙන් ගමන් කිරීමට තම දිගු හා සිහින් සිරුර භාවිතා කරයි. ශ්‍රී ලංකාවේ කොළ වැල් සර්පයාගේ ආහාරය ප්‍රධාන වශයෙන් කටුස්සන්, ගෙම්බන් සහ කෘමීන් වැනි කුඩා සතුන්ගෙන් සමන්විත වේ. ගොදුර සොයා ගැනීමට එහි විශිෂ්ට ඇස් පෙනීම භාවිතා කරන අතර පසුව එය අල්ලා ගැනීමට ඉක්මනින් පහර දෙයි. මෙම විශේෂය එහි අද්විතීය දඩයම් ශිල්පීය ක්‍රමය සඳහා ප්‍රසිද්ධය, එහි දී එය තම ගොදුරට සෙමින් ළඟා වන අතර පසුව ඔහුව අල්ලා ගැනීම සඳහා හදිසියේම පෙනහළු කරයි. ශ්‍රී ලංකාවේ කොළ වැල් සර්පයන් සාමාන්‍යයෙන් හුදකලා සතුන් වන අතර බොහෝ විට තනිව දක්නට ලැබේ.',
    imageUrl1: 'assets/snakes/ahaetulla_1.png',
    imageUrl2: 'assets/snakes/ahaetulla_2.png',
  ),
  AnimalDetails(
    commonName: 'Sri Lankan cat snake',
    scientificName: 'Boiga trigonata',
    imageUrl: 'assets/snakes/boiga_trigonata_1.png',
    venomousLevel: 'Mildly Venomous',
    description:
        '- Boiga trigonata, also known as the Sri Lankan cat snake, is a species of mildly venomous snake found in Sri Lanka and parts of southern India. This species can grow up to 1.2 meters (4 feet) in length and has a slender body with a triangular-shaped head. The coloration of the snake can vary, but typically it has a brown or grayish-brown base color with dark brown or black bands running down the length of its body. The Sri Lankan cat snake is primarily active at night and feeds on a diet of small mammals, birds, and lizards. Despite its venomous nature, its venom is not considered to be highly toxic to humans and is not generally considered to be a significant threat to human health. Overall, Boiga trigonata is a fascinating and important species in its native habitat, playing an important role in the local ecosystem as a predator and helping to control populations of small animals.',
    appearance:
        '- Boiga trigonata has a slender body with a triangular-shaped head. The body of this snake can grow up to 1.2 meters (4 feet) in length. The coloration of the snake can vary, but typically it has a brown or grayish-brown base color with dark brown or black bands running down the length of its body. The bands can be irregular in shape and vary in thickness. The scales of the snake are smooth and glossy in appearance. The eyes of Boiga trigonata are large and round, with vertical pupils. They are located towards the front of the head, giving the snake good depth perception and the ability to accurately strike at prey.',
    geographicLocation:
        '- Sri Lankan cat snake, is found primarily in Sri Lanka and parts of southern India. In Sri Lanka, this species is widespread and can be found in a variety of habitats, including rainforests, wetlands, and agricultural areas. In India, it is found in the Western Ghats, a mountain range along the western coast of the country. The Sri Lankan cat snake is generally not found at very high elevations, and is more commonly found in lowland areas up to around 600 meters (2000 feet) above sea level. It is not known to occur outside of its natural range in the wild.',
    lifeAndHabits:
        '- Boiga trigonata, the Sri Lankan cat snake, is primarily nocturnal and arboreal, meaning it spends most of its time in trees. During the day, it can often be found hiding in tree crevices or under loose bark.Reproduction in Boiga trigonata occurs through sexual reproduction, with males competing for access to females during the breeding season. Females typically lay a clutch of 6-12 eggs, which hatch after an incubation period of around 60-70 days.Overall, Boiga trigonata plays an important role in its ecosystem as a predator, helping to control populations of small animals. Despite its venomous nature, it is generally not considered to be a significant threat to human health.',
    sinhalaCommonName: 'රෑන්-මාපිලා',
    sinhalaScientificName: 'Boiga trigonata',
    sinhalaVenomousLevel: 'මද විෂ සහිත',
    sinhalaDescription:
        'ශ්‍රී ලංකාවේ බළල් සර්පයා ලෙසින්ද හැඳින්වෙන බොයිගා ට්‍රයිගොනාටා යනු ශ්‍රී ලංකාවේ සහ දකුණු ඉන්දියාවේ සමහර ප්‍රදේශවල දක්නට ලැබෙන මෘදු විෂ සහිත සර්ප විශේෂයකි. මෙම විශේෂය දිග මීටර් 1.2 (අඩි 4) දක්වා වර්ධනය විය හැකි අතර ත්රිකෝණාකාර හිසක් සහිත සිහින් සිරුරක් ඇත. සර්පයාගේ වර්ණය වෙනස් විය හැක, නමුත් සාමාන්‍යයෙන් එය දුඹුරු හෝ අළු-දුඹුරු පාදක වර්ණයකින් යුක්ත වන අතර තද දුඹුරු හෝ කළු පටි උගේ සිරුරේ දිගට දිව යයි. ශ්‍රී ලංකාවේ බළල් සර්පයා ප්‍රධාන වශයෙන් රාත්‍රියේ ක්‍රියාකාරී වන අතර කුඩා ක්ෂීරපායින්, කුරුල්ලන් සහ කටුස්සන්ගේ ආහාර වේලක් අනුභව කරයි. එහි විෂ සහිත ස්වභාවය තිබියදීත්, එහි විෂ මිනිසුන්ට ඉතා විෂ සහිත ලෙස නොසැලකෙන අතර සාමාන්‍යයෙන් මිනිස් සෞඛ්‍යයට සැලකිය යුතු තර්ජනයක් ලෙස නොසැලකේ.',
    sinhalaAppearance:
        '- ත්‍රිකෝණාකාර හිසක් සහිත සිහින් සිරුරක් ඇත. මෙම සර්පයාගේ සිරුර දිග මීටර් 1.2 (අඩි 4) දක්වා වර්ධනය විය හැක. සර්පයාගේ වර්ණය වෙනස් විය හැක, නමුත් සාමාන්‍යයෙන් එය දුඹුරු හෝ අළු-දුඹුරු පාදක වර්ණයකින් යුක්ත වන අතර තද දුඹුරු හෝ කළු පටි උගේ සිරුරේ දිගට දිව යයි. පටි හැඩයෙන් අක්රමවත් විය හැකි අතර ඝනකම වෙනස් වේ. සර්පයාගේ කොරපොතු පෙනුමෙන් සිනිඳු හා දිලිසෙන ය. Boiga trigonata ගේ ඇස් විශාල සහ වටකුරු, සිරස් ශිෂ්‍යයන් සහිත ය. ඒවා හිසේ ඉදිරිපස දෙසට පිහිටා ඇති අතර, සර්පයාට හොඳ ගැඹුරක් දැනීම සහ ගොදුරට නිවැරදිව පහර දීමේ හැකියාව ලබා දෙයි.',
    sinhalaGeographicLocation:
        '- ශ්‍රී ලංකා බළල් සර්පයා මූලික වශයෙන් ශ්‍රී ලංකාවේ සහ දකුණු ඉන්දියාවේ ප්‍රදේශ වල දක්නට ලැබේ. ශ්‍රී ලංකාවේ, මෙම විශේෂය බහුලව පැතිරී ඇති අතර වැසි වනාන්තර, තෙත්බිම් සහ කෘෂිකාර්මික ප්‍රදේශ ඇතුළු විවිධ වාසස්ථානවල දක්නට ලැබේ. ඉන්දියාවේ එය බටහිර ඝාට්ස් හි දක්නට ලැබේ, එය රටේ බටහිර වෙරළ තීරයේ කඳු වැටියකි. ශ්‍රී ලංකාවේ බළල් සර්පයා සාමාන්‍යයෙන් ඉතා ඉහළ උන්නතාංශවල දක්නට නොලැබෙන අතර, මුහුදු මට්ටමේ සිට මීටර් 600 (අඩි 2000) පමණ ඉහළ පහත් බිම් ප්‍රදේශවල බහුලව දක්නට ලැබේ. එය ස්වභාවික පරාසයෙන් පිටත වනයේ සිදු වන බව නොදනී.',
    sinhalaLifeAndHabits:
        '- ශ්‍රී ලාංකික බළල් සර්පයා, මූලික වශයෙන් නිශාචර සහ වෘක්ෂලතාදියෙකු වන අතර, එයින් අදහස් වන්නේ එය වැඩි කාලයක් ගස්වල ගත කරන බවයි. දිවා කාලයේදී, එය බොහෝ විට ගස් කුහරවල හෝ ලිහිල් පොත්ත යට සැඟවී සිටිනු දැකිය හැකිය. බෝයිගා ට්‍රයිගෝනාටා හි ප්‍රජනනය ලිංගික ප්‍රජනනය හරහා සිදු වේ, අභිජනන සමයේදී පිරිමින් ගැහැණු සතුන් සඳහා ප්‍රවේශය සඳහා තරඟ කරයි. ගැහැනු සතා සාමාන්‍යයෙන් බිත්තර 6-12 අතර බිත්තර පොකුරක් දමයි, ඒවා දින 60-70 ක පමණ පුර්ව ලියාපදිංචි තක්සේරු කාලයකින් පසුව පැටවුන් බිහි කරයි. සමස්තයක් වශයෙන්, Boiga trigonata එහි පරිසර පද්ධතිය තුළ විලෝපිකයෙකු ලෙස වැදගත් කාර්යභාරයක් ඉටු කරයි, කුඩා සතුන්ගේ ගහනය පාලනය කිරීමට උපකාරී වේ. එහි විෂ සහිත ස්වභාවය තිබියදීත්, එය සාමාන්යයෙන් මිනිස් සෞඛ්යයට සැලකිය යුතු තර්ජනයක් ලෙස නොසැලකේ.',
    imageUrl1: 'assets/snakes/boiga_trigonata_2.png',
    imageUrl2: 'assets/snakes/boiga_trigonata_3.png',
  ),
  // from there (Krishani)
  AnimalDetails(
    commonName: 'Buff Striped Keelback or Indian water snake.',
    scientificName: 'Amphiesma Stolatum',
    imageUrl: 'assets/snakes/amphiesma_stolatum_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '-  Amphiesma stolatum is a nonvenomous snake in the family Colubridae, which means it lacks venom glands and is not considered dangerous to humans.\n And Amphiesma stolatum is a medium-sized snake that typically grows to around 70-80 centimeters in length, but can reach up to 1 meter. It is a semi-aquatic snake and is often found near freshwater bodies such as ponds, streams, and marshes.\n  The keelback is active during the day and feeds primarily on fish, frogs, and other small aquatic animals.',
    appearance:
        '- The keelback has a slender body that is flattened from side to side, which allows it to move easily through water.\n  Its head is distinct from its body, and it has round pupils and a moderately long tail. \n The scales on the keelback\'s back are keeled, which means they have a raised ridge down the center, giving it a rough texture. The keelback\'s coloration is typically olive-green or brown on the dorsal side, with a yellow or white ventral side. \n Some individuals may have darker spots or stripes on their body, while others may have a uniform color. \n The back of the keelback\'s neck has a prominent keeled scale, which is a distinguishing feature of this species.',
    geographicLocation:
        '- Amphiesma stolatum is found in various parts of South Asia, including India, Sri Lanka, Bangladesh, Nepal, and Pakistan. ',
    lifeAndHabits:
        '- The keelback is a semi-aquatic snake that is often found near freshwater bodies. \n It is active during the day and feeds primarily on fish, frogs, and other small aquatic animals. \n The keelback is not considered dangerous to humans and is often kept as a pet in some parts of its range.',
    sinhalaCommonName: 'අහරකුක්කා',
    sinhalaScientificName: 'Amphiesma Stolatum',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- අහරකුක්කා යනු Colubridae පවුලේ විෂ නොවන සර්පයෙකි, එනම් විෂ ග්‍රන්ථි නොමැති අතර මිනිසුන්ට භයානක යැයි නොසැලකේ.\n තවද අහරකුක්කා සාමාන්‍යයෙන් සෙන්ටිමීටර 70-80 පමණ දිගට වැඩෙන මධ්‍යම ප්‍රමාණයේ සර්පයෙකි. නමුත් මීටර් 1 ක් දක්වා ළඟා විය හැකිය. අර්ධ ජලජ සර්පයෙකු වන අතර බොහෝ විට පොකුණු, ඇළ දොළ සහ වගුරු බිම් වැනි මිරිදිය ජලාශ අසල දක්නට ලැබේ.\n මොවුන් දිවා කාලයේ ක්‍රියාකාරී වන අතර ප්‍රධාන වශයෙන් මාළු, ගෙම්බන් සහ අනෙකුත් කුඩා ජලජ සතුන්ගෙන් පෝෂණය වේ.',
    sinhalaAppearance:
        '- අහරකුක්කා සතුව සිහින් සිරුරක් ඇති අතර එය ජලය හරහා පහසුවෙන් ගමන් කිරීමට ඉඩ සලසයි. \n සිරුරේ පිටුපස කොරපොතු ඇති අතර, එයින් රළු බවක් ලබා දෙයි. අහර කුක්කාගේ වර්ණය සාමාන්‍යයෙන් පෘෂ්ඨීය පැත්තේ ඔලිව්-කොළ හෝ දුඹුරු, කහ හෝ සුදු කශේරුකා පැත්තක් ඇත. \n සමහර විශේෂ වල සිරුරේ තද පැහැති ලප හෝ ඉරි තිබිය හැකි අතර අනෙක් විශේෂ වල ඒකාකාර වර්ණයක් තිබිය හැක. \n ගෙලෙහි පිටුපස කැපී පෙනෙන කොරපොතු පරිමාණයක් ඇත, එය මෙම විශේෂයේ කැපී පෙනෙන ලක්ෂණයකි. ',
    sinhalaGeographicLocation:
        '- ඉන්දියාව, ශ්‍රී ලංකාව, බංගලාදේශය, නේපාලය සහ පකිස්ථානය ඇතුළු දකුණු ආසියාවේ විවිධ ප්‍රදේශ වල අහර කුක්කා දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- අහරකුක්කා බොහෝ විට මිරිදිය ජලාශ අසල දක්නට ලැබෙන අර්ධ ජලජ සර්පයෙකි. \n මොවුන් දිවා කාලයේ ක්‍රියාකාරී වන අතර ප්‍රධාන වශයෙන් මාළු, ගෙම්බන් සහ අනෙකුත් කුඩා ජලජ සතුන් මත පෝෂණය වේ. \n අහරකුක්කා මිනිසුන්ට අනතුරුදායක ලෙස නොසැලකෙන අතර බොහෝ විට සමහර විශේෂ සුරතල් සතෙකු ලෙසද ඇති කරයි.',
    imageUrl1: 'assets/snakes/amphiesma_stolatum_2.jpg',
    imageUrl2: 'assets/snakes/amphiesma_stolatum_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Ornate Flying Snake.',
    scientificName: 'Chrysopelea Ornata',
    imageUrl: 'assets/snakes/chrysopelea_ornata_1.jpg',
    venomousLevel: 'Mildly Venomous (not considered dangerous)',
    description:
        '- Chrysopelea ornata is a species of arboreal snake that is capable of gliding through the air. It is slender and can grow up to 1.2 meters in length. \n Currently, three subspecies are recognized.  The snake\'s body is flattened, with a triangular head that is distinct from its body. \n Its scales are smooth and shiny, giving it a bright and iridescent appearance. ',
    appearance:
        '- Chrysopelea ornata is easily recognized by its unique, brightly-colored pattern. \n It has a golden or yellowish-brown background color, with black crossbands that form an intricate pattern on its back. The underside of the snake is typically white or cream-colored.',
    geographicLocation:
        '- Chrysopelea ornata is found in Southeast Asia, including Sri Lanka, India,  Thailand, Vietnam, Cambodia, and Laos. ',
    lifeAndHabits:
        '- Chrysopelea ornata is an arboreal species that spends most of its time in trees. It is primarily diurnal, which means it is active during the day. The ornate flying snake is a gliding snake, meaning it is able to launch itself from a high point and glide through the air, controlling its direction with its body. The snake feeds primarily on lizards, frogs, and other small prey. It is generally a shy and non-aggressive species. ',
    sinhalaCommonName: 'මල්සරා',
    sinhalaScientificName: 'Chrysopelea Ornata',
    sinhalaVenomousLevel: 'මද විෂ සහිත (දරුණු ලෙස නොසැලකේ)',
    sinhalaDescription:
        '- මල්සරා යනු වාතය හරහා ලිස්සා යාමේ හැකියාව ඇති සර්ප විශේෂයකි. එය සිහින් සහ දිග මීටර් 1.2 දක්වා වර්ධනය විය හැක. \n දැනට, උප විශේෂ තුනක් හඳුනාගෙන ඇත. සර්පයාගේ ශරීරය සමතලා වන අතර, උගේ ශරීරයෙන් වෙනස් වූ ත්‍රිකෝණාකාර හිසක් ඇත. \n එහි කොරපොතු සිනිඳු සහ දිලිසෙන අතර, එය දීප්තිමත් සහ වර්ණවත් පෙනුමක් ලබා දෙයි.  ',
    sinhalaAppearance:
        '- මොවුන්ගේ අද්විතීය, දීප්තිමත්-වර්ණ රටාව මගින් පහසුවෙන් හඳුනාගත හැකිය. සිරුර පිටුපස සංකීර්ණ රටාවක් සාදන කළු හරස් පටි සහිත, රන්වන් හෝ කහ-දුඹුරු පසුබිම් වර්ණයක් ඇත. සර්පයාගේ සිරුර යටි පැත්ත සාමාන්‍යයෙන් සුදු හෝ ක්‍රීම් වර්ණයක් ඇත.',
    sinhalaGeographicLocation:
        '- මල්සරා ශ්‍රී ලංකාව, ඉන්දියාව, තායිලන්තය, වියට්නාමය, කාම්බෝජය සහ ලාඕසය ඇතුළු අග්නිදිග ආසියාවේ දක්නට ලැබේ. ',
    sinhalaLifeAndHabits:
        '- මල්සරා ගස්වල වැඩි කාලයක් ගත කරන සර්ප විශේෂයකි. මොවුන් දිවා කාලයේ ක්‍රියාකාරී වන අතර වේ, මල්සරා වාතය හරහා ලිස්සා යන සර්පයෙකි, එනම් උස් ස්ථානයක සිට වාතය හරහා ලිස්සා යාමට හැකි වන අතර එහි දිශාව තම ශරීරය සමඟ පාලනය කරයි. මොවුන් මූලික වශයෙන් කටුස්සන්, ගෙම්බන් සහ අනෙකුත් කුඩා ගොදුරු මත පෝෂණය වේ. එය සාමාන්‍යයෙන් ආක්‍රමණශීලී නොවන විශේෂයකි.',
    imageUrl1: 'assets/snakes/chrysopelea_ornata_2.jpg',
    imageUrl2: 'assets/snakes/chrysopelea_ornata_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Indian wolf snake or Indian ground snake.',
    scientificName: 'Lycodon Aulicus',
    imageUrl: 'assets/snakes/lycodon_aulicus_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '-Lycodon aulicus is a small, slender species of snake that typically grows to be around 40-50 cm in length. \n It has a round, tapered head and a slender body that is cylindrical in shape. The scales on its body are smooth, and it has a glossy appearance. ',
    appearance:
        '- The Indian wolf snake is typically brown or grayish-brown in color, with darker brown or black stripes that run the length of its body. \n It has a white or yellowish underside, and its eyes are relatively large compared to its head size. ',
    geographicLocation:
        '- Lycodon aulicus is found in various parts of South Asia, including India, Nepal, Sri Lanka, and Bangladesh. It is a ground-dwelling species that is typically found in forested areas, as well as agricultural fields and other open habitats. ',
    lifeAndHabits:
        '- Lycodon aulicus is primarily nocturnal, which means it is active at night. It feeds on a variety of prey, including rodents, lizards, and other small animals. The Indian wolf snake is oviparous, meaning it lays eggs to reproduce. It is generally a non-aggressive species and will typically flee when threatened rather than attack. However, it may bite if cornered or handled, so it is important to exercise caution around any wild animal.',
    sinhalaCommonName: 'අළු-රදනකයා',
    sinhalaScientificName: 'Lycodon Aulicus',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- අළු-රදනකයා යනු සාමාන්‍යයෙන් සෙන්ටිමීටර 40-50ක් පමණ දිගට වැඩෙන කුඩා සිහින් විෂ නොවන සර්ප විශේෂයකි. මොවුන්ට  සිලින්ඩරාකාර හැඩයෙන් යුත් සිහින් සිරුරක් ඇත. උගේ සිරුරේ කොරපොතු සිනිඳු වන අතර එය දිලිසෙන පෙනුමක් ඇත.',
    sinhalaAppearance:
        '- අළු-රදනකයා සාමාන්‍යයෙන් දුඹුරු හෝ අළු-දුඹුරු පැහැයෙන් යුක්ත වන අතර තද දුඹුරු හෝ කළු ඉරි උගේ සිරුර දිගේ දිව යයි. එහි සුදු හෝ කහ පැහැති යටි පැත්තක් ඇති අතර, එහි හිසෙහි ප්‍රමාණයට සාපේක්ෂව ඇස් විශාලය.',
    sinhalaGeographicLocation:
        '- අළු-රදනකයා  ඉන්දියාව, නේපාලය, ශ්‍රී ලංකාව සහ බංග්ලාදේශය ඇතුළු දකුණු ආසියාවේ විවිධ ප්‍රදේශවල දක්නට ලැබේ. එය සාමාන්‍යයෙන් වනාන්තර ප්‍රදේශවල මෙන්ම කෘෂිකාර්මික ක්ෂේත්‍රවල සහ වෙනත් විවෘත වාසස්ථානවල දක්නට ලැබෙන විශේෂයකි.',
    sinhalaLifeAndHabits:
        '- අළු-රදනකයා   ප්‍රධාන වශයෙන් නිශාචර වන අතර එයින් අදහස් වන්නේ එය රාත්‍රියේ ක්‍රියාකාරී බවයි. එය මීයන්, කටුස්සන් සහ අනෙකුත් කුඩා සතුන් ඇතුළු විවිධ ගොදුරු මත පෝෂණය වේ. මොවුන් ඩිම්බකෝෂ සහිත ය, එනම් එය ප්‍රජනනය සඳහා බිත්තර දමයි. එය සාමාන්‍යයෙන් ආක්‍රමණශීලී නොවන විශේෂයක් වන අතර සාමාන්‍යයෙන් පහර දීමට වඩා තර්ජනයට ලක් වූ විට පලා යයි.',
    imageUrl1: 'assets/snakes/lycodon_aulicus_2.jpg',
    imageUrl2: 'assets/snakes/lycodon_aulicus_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Indian Rat Snake',
    scientificName: 'Ptyas mucosa',
    imageUrl: 'assets/snakes/ptyas_mucosa_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '- Ptyas mucosa, commonly known as the oriental ratsnake, Indian rat snake, darash or dhaman, is a common non-venomous species of colubrid snake found in parts of South and Southeast Asia. Dhamans are large snakes. \n Typical mature total length is around 1.5 to 1.95 m though some exceed 2 m.',
    appearance:
        '- Indian Rat Snakes have a distinct pattern on their body, consisting of dark spots or blotches that run along their back, which often fades as they mature. \n The underside of the snake is cream-colored, and the eyes are large and black with round pupils.',
    geographicLocation:
        '-  Indian Rat Snakes are found in South Asia, including India, Nepal, Bangladesh, Sri Lanka, and Pakistan. \n They inhabit a variety of environments, including forests, grasslands, agricultural lands, and urban areas.',
    lifeAndHabits:
        '-  Indian Rat Snakes are diurnal and highly active during the day. They are agile climbers and excellent swimmers, allowing them to move through various habitats. \n They feed on small mammals, birds, and reptiles, including rats, mice, lizards, and frogs. They are also known to be highly adaptable and can survive in human-altered environments, including agricultural fields and urban areas. Indian Rat Snakes are considered beneficial to humans because they help control rodent populations.',
    sinhalaCommonName: 'ගැරඬියා',
    sinhalaScientificName: 'Ptyas mucosa',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- ගැරඬියා, සාමාන්‍යයෙන් oriental ratsnake, Indian rat snake, darash හෝ dhaman ලෙස හඳුන්වනු ලබන අතර, දකුණු සහ අග්නිදිග ආසියාවේ ප්‍රදේශ වල දක්නට ලැබෙන පොදු විෂ නොවන colubrid සර්ප විශේෂයකි. ධමන් යනු විශාල සර්පයන් ය. සාමාන්‍ය පරිණත සම්පූර්ණ දිග මීටර් 1.5 සිට 1.95 දක්වා වන නමුත් සමහරක් මීටර් 2 ඉක්මවයි.',
    sinhalaAppearance:
        '-  ගැරඬියන්ගේ ශරීරය මත වෙනස් රටාවක් ඇත, ඔවුන්ගේ පිටුපස දිගේ දිවෙන අඳුරු ලප ඇත, ඒවා බොහෝ විට පරිණත වන විට මැකී යයි. සර්පයාගේ යටි පැත්ත ක්‍රීම් පැහැයෙන් යුක්ත වන අතර ඇස් කළු විශාල වන අතර වටකුරු ය.',
    sinhalaGeographicLocation:
        '- ගැරඬියන් ඉන්දියාව, නේපාලය, බංග්ලාදේශය, ශ්‍රී ලංකාව සහ පකිස්ථානය ඇතුළු දකුණු ආසියාවේ දක්නට ලැබේ. ඔවුන් වනාන්තර, තණබිම්, කෘෂිකාර්මික ඉඩම් සහ නාගරික ප්‍රදේශ ඇතුළු විවිධ පරිසරවල වාසය කරයි.',
    sinhalaLifeAndHabits:
        '- ගැරඬියන් දිවා කාලයේ ඉතා ක්‍රියාශීලී වේ. ඔවුන් කඩිසර කඳු නගින්නන් සහ විශිෂ්ට පිහිනුම්කරුවන් වන අතර විවිධ වාසස්ථාන හරහා ගමන් කිරීමට ඔවුන්ට ඉඩ සලසයි. ඔවුන් මීයන්, කටුස්සන් සහ ගෙම්බන් ඇතුළු කුඩා ක්ෂීරපායින්, කුරුල්ලන් සහ උරගයින්ගෙන් පෝෂණය වේ. මොවුන්ට හොදින් අනුවර්තනය විය හැකි අතර කෘෂිකාර්මික ක්ෂේත්‍ර සහ නාගරික ප්‍රදේශ ඇතුළුව මිනිසා විසින් වෙනස් කරන ලද පරිසරවල නොනැසී පැවතිය හැකිය. \n ගැරඬියන් මිනිසුන්ට ප්‍රයෝජනවත් යැයි සැලකේ, මන්ද ඔවුන් මීයන්ගේ ගහනය පාලනය කිරීමට උපකාරී වේ.',
    imageUrl1: 'assets/snakes/ptyas_mucosa_2.jpg',
    imageUrl2: 'assets/snakes/ptyas_mucosa_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Indian python, or Indian rock python',
    scientificName: 'Python Molurus',
    imageUrl: 'assets/snakes/python_molurus_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '- Indian pythons are one of the largest snakes in the world, with adults capable of growing up to 6 meters in length. \n They have a broad, muscular body and a large, triangular head. Their skin is covered in small, smooth scales that vary in color from light tan to dark brown, with dark, irregular blotches that run along their back and sides.',
    appearance:
        '-  Indian pythons have a distinctive pattern on their skin, consisting of a series of dark, blotchy patches on a lighter background. \n The underside of the snake is typically cream-colored or yellow, and their eyes are relatively small compared to their large head.',
    geographicLocation:
        '- Indian pythons are native to the Indian subcontinent, including India, Nepal, Bhutan, and Pakistan. They inhabit a range of environments, including forests, grasslands, agricultural areas, and urban areas.',
    lifeAndHabits:
        '- Indian pythons are primarily nocturnal and spend much of their time hiding in crevices or underground. They are constrictors and prey on a variety of animals, including rodents, birds, reptiles, and mammals. \n They are ambush predators and will wait patiently for their prey before striking. Indian pythons are also known for their ability to swim and are often found near water sources. They are considered an important species in their native habitat and are protected under Indian law.',
    sinhalaCommonName: 'පිඹුරා',
    sinhalaScientificName: 'Python Molurus',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- පිඹුරන් ලොව විශාලතම සර්පයන්ගෙන් එකකි, පරිණත පිඹුරන් දිග මීටර් 6 ක් දක්වා වර්ධනය වේ. \n ඔවුන්ට පුළුල්, මාංශපේශී ශරීරයක් සහ විශාල, ත්‍රිකෝණාකාර හිසක් ඇත. ඔවුන්ගේ සම කුඩා සිනිඳු කොරපොතු වලින් ආවරණය වී ඇති අතර ඒවා ලා දුඹුරු සිට තද දුඹුරු දක්වා වෙනස් වන අතර ඒවායේ පිටුපස සහ පැති දිගේ දිවෙන අඳුරු අක්‍රමවත් ලප ඇත.',
    sinhalaAppearance:
        '- පිඹුරන්ගේ සමෙහි සුවිශේෂී රටාවක් ඇති අතර, ළා පැහැති පසුබිමක අඳුරු, පැල්ලම් සහිත පැල්ලම් මාලාවකින් සමන්විත වේ. \n සර්පයාගේ යටි පැත්ත සාමාන්‍යයෙන් ක්‍රීම් හෝ කහ පැහැති වන අතර විශාල හිසට සාපේක්ෂව ඔවුන්ගේ ඇස් කුඩා වේ.',
    sinhalaGeographicLocation:
        '- පිඹුරන් ඉන්දියාව, නේපාලය, භූතානය සහ පකිස්ථානය ඇතුළු ඉන්දියානු උපමහාද්වීපය නිජබිම කරගෙන වාසය කරයි . ඔවුන් වනාන්තර, තණබිම්, කෘෂිකාර්මික ප්‍රදේශ සහ නාගරික ප්‍රදේශ ඇතුළු පරිසර පරාසයක වාසය කරයි.',
    sinhalaLifeAndHabits:
        '- පිඹුරන් ප්‍රධාන වශයෙන් නිශාචර වන අතර ඔවුන් වැඩි කාලයක් කුහරවල හෝ පොළව යට සැඟවී සිටියි. ඔවුන් මීයන්, කුරුල්ලන්, උරගයින් සහ ක්ෂීරපායින් ඇතුළු විවිධ සතුන් ගොදුරු කර ගනී. ඔවුන් සැඟවී සිටින විලෝපිකයන් වන අතර පහර දීමට පෙර ඔවුන්ගේ ගොදුර සඳහා ඉවසිලිවන්තව බලා සිටියි. පිඹුරන් පිහිනීම සඳහා ද ප්‍රසිද්ධ වන අතර බොහෝ විට ජල මූලාශ්‍ර අසල දක්නට ලැබේ. \n ඔවුන් ඔවුන්ගේ දේශීය වාසභූමියේ වැදගත් විශේෂයක් ලෙස සලකනු ලබන අතර ඉන්දියානු නීතිය යටතේ ආරක්ෂා කර ඇත.',
    imageUrl1: 'assets/snakes/python_molurus_2.jpg',
    imageUrl2: 'assets/snakes/python_molurus_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Sri Lankan green pit viper',
    scientificName: 'Trimeresurus trigonocephalus',
    imageUrl: 'assets/snakes/trimeresurus_trigonocephalus_1.jpg',
    venomousLevel: 'Mildly Venomous',
    description:
        '- Sri Lankan pit vipers are relatively small snakes, with adults growing to a length of around 60-70 cm. They have a triangular-shaped head, hence the name trigonocephalus, and a slender body covered in keeled scales. \n Their coloration can vary, but they typically have a bright green or yellow-green body with darker green or black markings along their back and sides.',
    appearance:
        '-  Sri Lankan pit vipers have a distinctive coloration that helps them blend into their natural habitat of forested areas. They have vertical pupils and a pit organ between their eyes and nostrils that enables them to sense the heat of their prey. ',
    geographicLocation:
        '- Sri Lankan pit vipers are found in Sri Lanka and southern India, inhabiting a range of forested areas and often found near water sources.',
    lifeAndHabits:
        '- It is arboreal and nocturnal, occasionally descending to the ground in search of food such as lizards, frogs, small mammals, and birds. \n This sluggish pit viper is usually encountered on low shrubs during morning hours, but it mostly occupies in grasslands and rain forests. In mornings, it is seen to stay on top of trees to obtain sun rays to heat its body. It uses its tail to hold on to a tree branch. \n This is not a particularly defensive species, but if agitated, it will vibrate its tail tip, form a sinuous loop with the fore body, and lash and attempt to bite, seldom with a hissing sound. It produces live young mostly during June and july.',
    sinhalaCommonName: 'පලා පොළඟා',
    sinhalaScientificName: 'Trimeresurus trigonocephalus',
    sinhalaVenomousLevel: 'මද විෂ සහිත',
    sinhalaDescription:
        '- පලා පොළඟා යනු සාපේක්ෂව කුඩා සර්පයන් වන අතර පරිණත සතුන් සෙන්ටිමීටර 60-70ක් පමණ දිගට වැඩෙයි. ඔවුන්ට ත්‍රිකෝණාකාර හැඩැති හිසක් ඇත, එබැවින් ට්‍රයිගොනොසෙෆලස් යන නම ලැබී ඇත. \n සිරුර සිහින් කොරපොතු වලින් ආවරණය වී ඇත. ඔවුන්ගේ වර්ණය වෙනස් විය හැක, නමුත් සාමාන්යයෙන් ඔවුන්ගේ පිටුපස සහ පැති දිගේ තද කොළ හෝ කළු සලකුණු සහිත දීප්තිමත් කොළ හෝ කහ-කොළ ශරීරයක් ඇත.',
    sinhalaAppearance:
        '- ශ්‍රී ලංකාවේ පලා පොළඟුන්ට සුවිශේෂී වර්ණ ගැන්වීමක් ඇති අතර එය වනාන්තර ප්‍රදේශවල ඔවුන්ගේ ස්වභාවික වාසභූමියට මිශ්‍ර වීමට උපකාරී වේ. ඔවුන්ගේ ඇසේ කණීනිකාව සිරස් අතට ඇති අතර ඔවුන්ගේ ඇස් සහ නාස්පුඩු අතර වළේ අවයවයක් ඇත. එමඟින් ඔවුන්ගේ ගොදුරේ තාපය දැනීමට හැකි වේ.',
    sinhalaGeographicLocation:
        '- ශ්‍රී ලංකාවේ පලා පොළඟුන් ශ්‍රී ලංකාවේ සහ දකුණු ඉන්දියාවේ දක්නට ලැබෙන අතර, වනාන්තර ප්‍රදේශ කිහිපයක වාසය කරන අතර බොහෝ විට ජල මූලාශ්‍ර අසල දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- පලා පොළඟා නිශාචර වන අතර වෘක්ෂලතා මත වාසය කරයි. , කටුස්සන්, ගෙම්බන්, කුඩා ක්ෂීරපායින් සහ කුරුල්ලන් වැනි ආහාර සෙවීම සඳහා ඉඳහිට බිමට බැස යයි. සාමාන්‍යයෙන් උදය කාලයේදී පහත් පඳුරු මත දක්නට ලැබේ, නමුත් බොහෝ දුරට තණබිම් සහ වැසි වනාන්තරවල වාසය කරයි. උදය කාලයේදී තම සිරුර රත් කිරීමට හිරු කිරණ ලබා ගැනීම සඳහා ගස් මත රැඳී සිටිනු දක්නට ලැබේ.ගසක අත්තක් අල්ලා ගැනීමට එහි වලිගය භාවිතා කරයි. \n මෙය විශේෂ ආරක්‍ෂක උපක්‍රමයක් නොවේ, නමුත් කලබල වුවහොත්, උගේ වලිග තුඩ කම්පනය කර කලාතුරකින් හිස්සිං ශබ්දයක් සමඟ පහර දී දෂ්ට කිරීමට උත්සාහ කරයි. බොහෝ විට මොවුන් ජුනි සහ ජූලි මාසවලදී පැටවුන් බිහි කරයි.',
    imageUrl1: 'assets/snakes/trimeresurus_trigonocephalus_2.jpg',
    imageUrl2: 'assets/snakes/trimeresurus_trigonocephalus_3.jpg',
  ),
];

List<AnimalDetails> beesList = [
  AnimalDetails(
    commonName: 'Garden Bumble Bee',
    scientificName: 'Bombus hortorum',
    imageUrl: 'assets/bees/garden_bumble_bee_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '- Bombus hortorum, the garden bumblebee or small garden bumblebee, is a species of bumblebee found in most of Europe north to 70°N, as well as parts of Asia and New Zealand. \n It is distinguished from most other bumblebees by its long tongue used for feeding on pollen in deep-flowered plants. \n Accordingly, this bumblebee mainly visits flowers with deep corollae, such as deadnettles, ground ivy, vetches, clovers, comfrey, foxglove, and thistles. \n They have a good visual memory, which aids them in navigating the territory close to their habitat and seeking out food sources',
    appearance:
        '- This bumblebee has an oblong head and a very long tongue, about 15 mm (0.59 in), and in some cases even 20 mm (0.79 in). \n The tongue is so long that the bee often flies with it extended when collecting nectar. The queen is variable in size, with body lengths between 19 and 22 mm (0.75 and 0.87 in), and wing spans from 35 to 38 mm (1.4 to 1.5 in). \n The workers are almost as large, the larger ones overlapping the smaller queens. Their color is black with a yellow collar, a narrow yellow band on the scutellum, and a third yellow band on terga (abdominal segments) 1 and 2. \n The tail is white. Darker forms, with little yellow in their fur, are common. ',
    geographicLocation:
        '- Found in most of Europe north to 70°N, as well as parts of Asia and New Zealand.',
    lifeAndHabits:
        '- Garden Bumble Bees are active from early spring to late summer and can often be seen buzzing around flowers, collecting nectar and pollen. They build their nests in underground burrows or in abandoned rodent burrows. \n The queen bee will lay eggs, and the worker bees will tend to the young and gather food. In the fall, new queen bees and males are produced, and they will mate before the old queen dies and the colony comes to an end.',
    sinhalaCommonName: ' උද්‍යාන මී මැස්සා ',
    sinhalaScientificName: 'Bombus hortorum ',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- උද්‍යාන මී මැස්සා යනු යුරෝපයේ බොහෝ උතුරු ප්‍රදේශ වල මෙන්ම ආසියාවේ සහ නවසීලන්තයේ සමහර ප්‍රදේශවල දක්නට ලැබෙන මී මැස්සන් විශේෂයකි. \n මල් සහිත ශාකවල පරාග පෝෂණය කිරීම සඳහා භාවිතා කරන දිගු තුඩ නිසා මොවුන් අනෙකුත් බොහෝ මී මැස්සන්ගෙන් කැපී පෙනේ. \n ඒ අනුව, මෙම මී මැස්සන් ප්‍රධාන වශයෙන් පැමිණෙන්නේ ඩෙඩ්නෙට්ල්ස්, ග්‍රවුන්ඩ් අයිවි, වෙච්, ක්ලෝවර්, කොම්ෆ්‍රේ, ෆොක්ස්ග්ලෝව් වැනි ගැඹුරු කොරොල්ලා සහිත මල් වලටය. \n ඔවුන්ට හොඳ දෘශ්‍ය මතකයක් ඇති අතර, එය ඔවුන්ගේ වාසභූමියට ආසන්න භූමියේ සැරිසැරීමට සහ ආහාර ප්‍රභවයන් සෙවීමට උපකාරී වේ.',
    sinhalaAppearance:
        '- මෙම මී මැස්සාට දිගටි හිසක් සහ ඉතා දිගු තුඩක්, මිලිමීටර් 15 (අඟල් 0.59) පමණ වන අතර සමහර අවස්ථාවල මිලිමීටර් 20 (අඟල් 0.79) පවා වේ. \n තුඩ කොතරම් දිගුද යත් මල් පැණි එකතු කරන විට එය දිගු කර පියාසර කරයි. රැජින ප්‍රමාණයෙන් විවිද වන අතර සිරුරේ දිග 19 සහ 22 mm (අඟල් 0.75 සහ 0.87) අතර පියාපත් 35 සිට 38 mm (අඟල් 1.4 සිට 1.5 දක්වා) දක්වා විහිදේ. \n කම්කරු මී මැස්සන් බොහෝ දුරට විශාල වේ. ඔවුන්ගේ වර්ණය කහ පැහැති පටියක් සහිත කළු උදර කොටස් වලින් සමන්විතය. \n වලිගය සුදු ය. ඔවුන්ගේ ලොම් වල කුඩා කහ සහිත අඳුරු කොටස් බහුලව දක්නට ලැබේ.',
    sinhalaGeographicLocation:
        '- යුරෝපයේ බොහෝ උතුරු ප්‍රදේශ වල මෙන්ම ආසියාවේ සහ නවසීලන්තයේ සමහර ප්‍රදේශවල දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- උද්‍යාන මී මැස්සන් වසන්තයේ මුල් භාගයේ සිට ගිම්හානයේ අග දක්වා ක්‍රියාකාරී වන අතර බොහෝ විට මල් වටා ඝෝෂා කරමින් මල් පැණි සහ පරාග එකතු කිරීම දැකිය හැකිය. ඔවුන් තම කූඩු ගොඩනඟන්නේ භූගත ගුහාවල හෝ අතහැර දැමූ මීයන්ගේ වලවල්වල ය. මී මැසි රැජින බිත්තර දමන අතර වැඩ කරන මී මැස්සන් පැටවුන්ට ආහාර රැස් කරයි. සරත් සෘතුවේ දී, නව රැජිනක් සහ පිරිමි මී මැස්සන් බිහි වන අතර, පැරණි රැජින මිය ගොස් ජනපදය අවසන් වීමට පෙර ඔවුන් සංසර්ගයේ යෙදෙනු ඇත.',
    imageUrl1: 'assets/bees/garden_bumble_bee_2.jpg',
    imageUrl2: 'assets/bees/garden_bumble_bee_3.jpg',
  ),
  AnimalDetails(
    commonName: 'violet carpenter bee',
    scientificName: 'Xylocopa violacea',
    imageUrl: 'assets/bees/violet_carpenter_bee_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '- The Violet Carpenter Bee is a large, solitary bee that is part of the Xylocopa genus. It has a robust body and is typically around 25-30 mm in length. The male and female of this species have different markings. \n The male has a yellow face and yellow markings on its thorax, while the female has a black face and a metallic violet-black thorax.',
    appearance:
        '- The Violet Carpenter Bee has a robust body and is typically around 25-30 mm in length. The male and female of this species have different markings. The male has a yellow face and yellow markings on its thorax, while the female has a black face and a metallic violet-black thorax. Both males and females have transparent wings.',
    geographicLocation:
        '- The Violet Carpenter Bee is found in many parts of Europe and North Africa, including the United Kingdom, Spain, Italy, and Morocco. They are typically found in wooded areas, gardens, and parks.',
    lifeAndHabits:
        '- Violet Carpenter Bees are solitary bees, meaning that they do not live in colonies like honeybees. They typically nest in dead wood, such as tree stumps or fallen logs. The female will excavate a tunnel in the wood and create individual cells for her eggs. The larvae feed on pollen and nectar before pupating and emerging as adults. \n These bees are important pollinators for many plants, and they play a vital role in maintaining healthy ecosystems. They are not aggressive towards humans, but like all bees, they will defend themselves if they feel threatened.',
    sinhalaCommonName: 'අම්බලන් පාලුවා',
    sinhalaScientificName: 'Xylocopa violacea',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '-  අම්බලන් පාලුවා යනු Xylocopa ගණයට අයත් වන විශාල තනි මී මැස්සෙකි. මොවුන්ට ශක්තිමත් ශරීරයක් ඇති අතර සාමාන්‍යයෙන් දිග 25-30 mm පමණ වේ. මෙම විශේෂයේ පිරිමි සහ ගැහැණු සතුන් විවිධ වේ. \n පිරිමි සතාට කහ පැහැති මුහුණක් සහ උගේ පපුවේ කහ සලකුණු ඇති අතර ගැහැණු සතාට කළු මුහුණක් සහ ලෝහමය වයලට්-කළු උරස් එකක් ඇත.',
    sinhalaAppearance:
        '- ශක්තිමත් සිරුරක් ඇති අතර සාමාන්‍යයෙන් දිග 25-30 mm පමණ වේ. මෙම විශේෂයේ පිරිමි සහ ගැහැණු සතුන් විවිධ වේ. පිරිමි සතාට කහ පැහැති මුහුණක් සහ උගේ පපුවේ කහ පැහැති සලකුණු ඇති අතර ගැහැණු සතාට කළු මුහුණක් සහ ලෝහමය වයලට්-කළු පපුවක් ඇත. පිරිමි සහ ගැහැණු යන දෙපිරිසටම විනිවිද පෙනෙන පියාපත් ඇත.',
    sinhalaGeographicLocation:
        '- එක්සත් රාජධානිය, ස්පාඤ්ඤය, ඉතාලිය සහ මොරොක්කෝව ඇතුළු යුරෝපයේ සහ උතුරු අප්‍රිකාවේ බොහෝ ප්‍රදේශවල දක්නට ලැබේ. මොවුන් සාමාන්‍යයෙන් වනාන්තරවල, උද්‍යානවල සහ උද්‍යානවල දක්නට ලැබේ. ',
    sinhalaLifeAndHabits:
        '- අම්බලන් පාලුවා යනු හුදකලා මී මැස්සන් වන අතර එයින් අදහස් කරන්නේ ඔවුන් මී මැස්සන් මෙන් ජනපදවල ජීවත් නොවන බවයි. ඔවුන් සාමාන්‍යයෙන් කූඩු තනන්නේ ගස් කඳන් හෝ වැටුණු දර වැනි දිරාපත් ලීවල ය. ගැහැණු සතා ලීයෙන් උමගක් හාරා බිත්තර දමයි. කීටයන් වැඩිහිටියෙකු ලෙස මතු වීමට පෙර පරාග සහ මල් පැණි අනුභව කරයි. \n මෙම මී මැස්සන් බොහෝ ශාක සඳහා වැදගත් පරාග වාහකයන් වන අතර සෞඛ්‍ය සම්පන්න පරිසර පද්ධතීන් පවත්වා ගැනීම සඳහා ඔවුන් ඉතා වැදගත් කාර්යභාරයක් ඉටු කරයි. ඔවුන් මිනිසුන්ට ආක්‍රමණශීලී නොවේ, නමුත් සියලුම මී මැස්සන් මෙන්, ඔවුන්ට තර්ජනයක් දැනෙනවා නම් ඔවුන් ආරක්ෂා වේ.',
    imageUrl1: 'assets/bees/violet_carpenter_bee_2.jpg',
    imageUrl2: 'assets/bees/violet_carpenter_bee_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Honey bee',
    scientificName: ' Apis mellifera ',
    imageUrl: 'assets/bees/honey_bee_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '-  Honey bees are social insects that belong to the family Apidae. They are important pollinators and producers of honey. \n Honey bees are known for their complex social behavior and communication, which includes the famous "waggle dance" used to communicate the location of food sources to other bees in the hive.',
    appearance:
        '-  Honey bees are small insects with a fuzzy, yellow and black striped body. They have two pairs of wings and six legs. \n The worker bees are about 12mm long, while the queen bee is slightly larger at around 15mm in length.',
    geographicLocation:
        '- Honey bees are found all over the world, except in Antarctica. They are native to Europe, Africa, and western Asia, \n  but have been introduced to many other parts of the world, including North and South America, Australia, and New Zealand.',
    lifeAndHabits:
        '- Honey bees are social insects that live in colonies or hives. The colony consists of a queen bee, worker bees, and drones. The worker bees are responsible for collecting nectar and pollen from flowers, which they use to produce honey and feed the colony. The queen bee is responsible for laying eggs and maintaining the colony\'s population. \n Drones are male bees that mate with the queen. Honey bees are active throughout the year, but are most active during the spring and summer months. They are important pollinators for many crops, and their honey is used for food and medicinal purposes.',
    sinhalaCommonName: 'මී මැස්සා',
    sinhalaScientificName: ' Apis mellifera',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- මී මැස්සන් Apidae පවුලට අයත් සමාජශීලී කෘමීන් වේ. ඔවුන් වැදගත් පරාග කාරක සහ මී පැණි නිෂ්පාදකයන් වේ. \n මී මැස්සන් ඔවුන්ගේ සංකීර්ණ සමාජ හැසිරීම් සහ සන්නිවේදනය සඳහා ප්‍රසිද්ධ වී ඇති අතර, මී මැස්සන් වදයේ සිටින අනෙකුත් මී මැස්සන්ට ආහාර ප්‍රභවයන් ඇති ස්ථානය සන්නිවේදනය කිරීමට භාවිතා කරන සුප්‍රසිද්ධ "වැගල් නැටුම" භාවිතා කරයි.',
    sinhalaAppearance:
        '- මී මැස්සන් යනු නොපැහැදිලි, කහ සහ කළු ඉරි සහිත ශරීරයක් සහිත කුඩා කෘමීන් ය. ඔවුන්ට පියාපත් යුගල දෙකක් සහ කකුල් හයක් ඇත. \n වැඩකාර මී මැස්සන් මිලිමීටර් 12ක් පමණ දිග වන අතර රැජින මී මැස්සන් තරමක් විශාල වන අතර දිග මිලිමීටර් 15ක් පමණ වේ.',
    sinhalaGeographicLocation:
        '- මී මැස්සන් ඇන්ටාක්ටිකාව හැර ලොව පුරා දක්නට ලැබේ. ඔවුන් යුරෝපය, අප්‍රිකාව සහ බටහිර ආසියාවේ නිජබිම වන නමුත් උතුරු සහ දකුණු ඇමරිකාව, ඕස්ට්‍රේලියාව සහ නවසීලන්තය ඇතුළු ලෝකයේ වෙනත් බොහෝ ප්‍රදේශවල හඳුනාගත හැක.',
    sinhalaLifeAndHabits:
        '- මී මැස්සන් යනු ජනපදවල හෝ වද වල ජීවත් වන සමාජ කෘමීන් ය. මෙම ජනපදය සමන්විත වන්නේ මී මැසි රැජිනක්, සේවක මී මැස්සන් සහ පිරිමි මී මැස්සන්ගෙන්ය. වැඩ කරන මී මැස්සන් මල් වලින් මල් පැණි සහ පරාග එකතු කිරීමේ වගකීම දරයි, ඔවුන් මී පැණි නිපදවීමට සහ ජනපදය පෝෂණය කිරීමට භාවිතා කරයි. රැජින බිත්තර දැමීම සහ ජනපදයේ ජනගහනය නඩත්තු කිරීම සඳහා වගකිව යුතුය. \n අනෙකුත් මී මැස්සන් රැජින සමඟ සංසර්ගයේ යෙදෙන පිරිමි මී මැස්සන් ය. මී මැස්සන් වසර පුරා ක්‍රියාකාරී වන නමුත් වසන්ත හා ගිම්හාන මාසවලදී වඩාත් ක්‍රියාකාරී වේ. මොවුන් බොහෝ භෝග සඳහා වැදගත් පරාගකාරක වන අතර, ඔවුන්ගේ මී පැණි ආහාර හා ඖෂධීය අරමුණු සඳහා යොදා ගනී.',
    imageUrl1: 'assets/bees/honey_bee_2.jpg',
    imageUrl2: 'assets/bees/honey_bee_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Pure Green Sweat Bee',
    scientificName: 'Augochlora pura',
    imageUrl: 'assets/bees/pure_green_sweat_bee_1.jpg',
    venomousLevel: 'Non-Venomous',
    description:
        '- The Pure Green Sweat Bee is a species of bee in the Halictidae family. They are a small, solitary bee that are typically found in grasslands and forested areas. \n They are known for their bright green metallic coloration.',
    appearance:
        '- The Pure Green Sweat Bee is a small bee, measuring about 6 to 9 mm in length. \n They are bright green in color, with a metallic sheen. They have short, hairy antennae and wings that are translucent.',
    geographicLocation:
        '- The Pure Green Sweat Bee is found in North America, specifically in the eastern United States and parts of Canada.',
    lifeAndHabits:
        '- The Pure Green Sweat Bee is a solitary bee, meaning that they do not live in colonies like honey bees. \n They are important pollinators for a variety of plants, including wildflowers and crops. They get their name from their habit of collecting salt and moisture from human sweat, as well as from the sweat of other animals. \n They typically build their nests in the ground or in rotting wood, and the female bee lays her eggs in small chambers within the nest. The Pure Green Sweat Bee is not typically aggressive, and they will only sting if they feel threatened.',
    sinhalaCommonName: 'Pure Green Sweat Bee',
    sinhalaScientificName: 'Augochlora pura',
    sinhalaVenomousLevel: 'විෂ සහිත නොවන',
    sinhalaDescription:
        '- Pure Green Sweat Bee යනු Halictidae පවුලේ මී මැසි විශේෂයකි. ඔවුන් සාමාන්‍යයෙන් තණබිම් සහ වනාන්තර ප්‍රදේශවල දක්නට ලැබෙන කුඩා, හුදකලා මී මැස්සෙකි. \n ඔවුන් ඔවුන්ගේ දීප්තිමත් කොළ පැහැති ලෝහමය වර්ණ සඳහා ප්‍රසිද්ධය.',
    sinhalaAppearance:
        '- Pure Green Sweat Bee යනු කුඩා මී මැස්සෙකි, දිග මිලිමීටර් 6 සිට 9 දක්වා වේ. මොවුන් ලෝහමය බැබළීමක් සහිත දීප්තිමත් කොළ පැහැති වේ. මොවුන්ට කෙටි, රෝම සහිත ඇන්ටෙනා සහ පාරභාසක පියාපත් ඇත.',
    sinhalaGeographicLocation:
        '- මෙම මී මැස්සන් විශේෂය උතුරු ඇමරිකාවේ, විශේෂයෙන් නැගෙනහිර එක්සත් ජනපදයේ සහ කැනඩාවේ සමහර ප්‍රදේශවල දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- Pure Green Sweat Bee යනු හුදකලා මී මැස්සෙකු වන අතර එයින් අදහස් කරන්නේ ඔවුන් අනෙකුත් මී මැස්සන් මෙන් ජනපදවල ජීවත් නොවන බවයි. වල් මල් සහ භෝග ඇතුළු විවිධ ශාක සඳහා ඒවා වැදගත් පරාගකාරක වේ. \n මිනිසුන්ගේ දහඩියෙන් මෙන්ම අනෙකුත් සතුන්ගේ දහඩියෙන් ලුණු සහ තෙතමනය එකතු කිරීමේ පුරුද්ද නිසා ඔවුන්ට ඔවුන්ගේ නම ලැබී ඇත. ඔවුන් සාමාන්‍යයෙන් තම කූඩු ගොඩනගන්නේ පොළොවේ හෝ දිරාගිය ලීවල වන අතර ගැහැණු මී මැස්සන් කූඩුව තුළ කුඩා කුටිවල බිත්තර දමයි. \n මොවුන් සාමාන්‍යයෙන් ආක්‍රමණශීලී නොවන අතර, ඔවුන් දෂ්ට කරන්නේ තර්ජනයක් යැයි හැඟේ නම් පමණි.',
    imageUrl1: 'assets/snakes/pure_green_sweat_bee_1.jpg',
    imageUrl2: 'assets/snakes/pure_green_sweat_bee_1.jpg',
  ),
];

List<AnimalDetails> waspList = [
  AnimalDetails(
    commonName: 'Asian giant hornet',
    scientificName: 'Vespa mandarinia',
    imageUrl: 'assets/wasp/asian_giant_hornet_1.jpg',
    venomousLevel: 'Deadly Venomous',
    description:
        '- The Asian Giant Hornet is one of the largest species of hornet in the world, measuring up to 5 cm in length. They have a distinctive orange head with large eyes and a black and yellow striped body. Their stingers are longer than those of other hornets, and they are known for their potent venom.',
    appearance:
        '-  The Asian Giant Hornet has a distinctive orange head with large eyes and a black and yellow striped body. They are one of the largest species of hornet in the world, measuring up to 5 cm in length.',
    geographicLocation:
        '- The Asian Giant Hornet is native to Asia, specifically Japan, Korea, and China. However, in recent years, they have been found in other parts of the world, including North America.',
    lifeAndHabits:
        '-  Asian Giant Hornets are social insects that live in colonies. The colonies can contain up to 700 individuals, with a queen at the center of the colony. They are known for their aggressive behavior, and they will attack humans and animals if they feel threatened. \n They are also known to attack and destroy honeybee colonies, making them a threat to beekeepers and the pollination of crops. Their diet consists of other insects, including honeybees, and they are known to be important predators in their ecosystem.',
    sinhalaCommonName: 'Asian giant hornet',
    sinhalaScientificName: 'Vespa mandarinia',
    sinhalaVenomousLevel: 'මාරාන්තික විෂ සහිත',
    sinhalaDescription:
        '- ලොව විශාලතම බඹර විශේෂ වලින් එකකි, දිග සෙන්ටිමීටර 5 ක් දක්වා වේ. ඔවුන් විශාල ඇස් සහ කළු සහ කහ ඉරි සහිත ශරීරයක් සහිත සුවිශේෂී තැඹිලි හිසක් ඇත. ඔවුන්ගේ දෂ්ට කිරීම් අනෙකුත් බඹරුන්ට වඩා විෂ වන අතර ඒවා ප්‍රබල විෂ සඳහා ප්‍රසිද්ධය.',
    sinhalaAppearance:
        '- විශාල ඇස් සහ කළු සහ කහ ඉරි සහිත ශරීරයක් සහිත සුවිශේෂී තැඹිලි හිසක් ඇත. ඔවුන් ලොව විශාලතම බඹර විශේෂ වලින් එකකි, දිග සෙන්ටිමීටර 5 ක් දක්වා වේ.',
    sinhalaGeographicLocation:
        '- ආසියාවේ, විශේෂයෙන්ම ජපානය, කොරියාව සහ චීනයයි. කෙසේ වෙතත්, මෑත වසරවලදී, ඔවුන් උතුරු ඇමරිකාව ඇතුළු ලෝකයේ වෙනත් ප්රදේශවලින් සොයාගෙන ඇත.',
    sinhalaLifeAndHabits:
        '- ජනපදවල ජීවත් වේ. යටත් ජනපදවල බඹරුන් 700ක් දක්වා සිටිය හැකි අතර, ජනපදයේ මධ්‍යයේ රැජිනක් සිටී. ඔවුන් ඔවුන්ගේ ආක්‍රමණශීලී හැසිරීම් සඳහා ප්‍රසිද්ධියක් උසුලන අතර, ඔවුන් තර්ජනයට ලක්වුවහොත් මිනිසුන්ට සහ සතුන්ට පහර දෙනු ඇත. මීමැසි ජනපදවලට පහර දී විනාශ කරන ඔවුන් මීමැසි පාලකයන්ට තර්ජනයක් වන අතර බෝග පරාගණයට ද තර්ජනයක් වේ. ඔවුන්ගේ ආහාර වේල මී මැස්සන් ඇතුළු අනෙකුත් කෘමීන්ගෙන් සමන්විත වන අතර ඔවුන් ඔවුන්ගේ පරිසර පද්ධතියේ වැදගත් විලෝපිකයන් ලෙස හැඳින්වේ.',
    imageUrl1: 'assets/wasp/asian_giant_hornet_2.jpg',
    imageUrl2: 'assets/wasp/asian_giant_hornet_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Blackjacket ',
    scientificName: 'Vespula consobrina',
    imageUrl: 'assets/wasp/blackjacket_1.jpg',
    venomousLevel: 'Mildly Venomous',
    description:
        '- Vespula consobrina, the blackjacket, is a species of stinging wasp in the family Vespidae. The Blackjacket is a type of wasp that is similar in appearance to other yellowjacket wasps. They have black and yellow stripes on their body, with a distinctive black marking on their face that gives them their common name. \n They are a social species, and their nests can contain up to several thousand individuals.',
    appearance:
        '- The Blackjacket is a black and yellow striped wasp with a distinctive black marking on its face.\n They are similar in appearance to other yellowjacket wasps. ',
    geographicLocation:
        '- The Blackjacket is found throughout much of North America, including the United States and Canada. \n They prefer to live in wooded areas and are also commonly found near human habitation. ',
    lifeAndHabits:
        '-  The Blackjacket is a social species, and their nests can contain up to several thousand individuals. \n They are known to be aggressive and will sting if they feel threatened. They are omnivores and will feed on a variety of food sources, including insects, fruit, and other sources of sugar. \n They are also attracted to human food and can be a nuisance at outdoor gatherings. Their nests are typically located in the ground or in trees, and they will defend their nests aggressively.',
    sinhalaCommonName: 'බ්ලැක් ජැකට්',
    sinhalaScientificName: 'Vespula consobrina',
    sinhalaVenomousLevel: 'මද විෂ සහිත',
    sinhalaDescription:
        '-  බ්ලැක් ජැකට් යනු අනෙකුත් කහ ජැකට් බඹරුන්ගේ පෙනුමට සමාන බඹර විශේෂයකි. ඔවුන්ගේ ශරීරයේ කළු සහ කහ ඉරි ඇති අතර, ඔවුන්ගේ මුහුණේ සුවිශේෂී කළු සලකුණක් ඇති අතර එමඟින් ඔවුන්ගේ පොදු නම ලබා දේ. ඔවුන් සමාජ විශේෂයක් වන අතර, ඔවුන්ගේ කූඩුවල බඹරුන් දහස් ගණනක් අඩංගු විය හැකිය.',
    sinhalaAppearance:
        '- බ්ලැක් ජැකට් යනු කළු සහ කහ ඉරි සහිත බඹරෙකු වන අතර මොවුන්ගේ මුහුණේ සුවිශේෂී කළු සලකුණක් ඇත. ඔවුන් අනෙකුත් කහ ජැකට් බඹරුන්ට පෙනුමෙන් සමාන ය. ',
    sinhalaGeographicLocation:
        '- බ්ලැක් ජැකට් එක්සත් ජනපදය සහ කැනඩාව ඇතුළු උතුරු ඇමරිකාවේ බොහෝ ප්‍රදේශවල දක්නට ලැබේ. ඔවුන් වනාන්තර ප්‍රදේශවල ජීවත් වීමට කැමැත්තක් දක්වන අතර මිනිස් වාසස්ථාන අසලද බහුලව දක්නට ලැබේ. ',
    sinhalaLifeAndHabits:
        '- බ්ලැක් ජැකට් යනු සමාජ විශේෂයක් වන අතර ඔවුන්ගේ කූඩුවල බඹරුන් දහස් ගණනක් අඩංගු විය හැකිය. ඔවුන් ආක්‍රමණශීලී වන අතර ඔවුන්ට තර්ජනයක් දැනේ නම් දෂ්ට කරයි. ඔවුන් සර්ව භක්ෂකයින් වන අතර කෘමීන්, පලතුරු සහ අනෙකුත් සීනි ප්‍රභවයන් ඇතුළු විවිධ ආහාර ප්‍රභවයන් මත පෝෂණය වේ. ඔවුන් මිනිස් ආහාර කෙරෙහි ද ආකර්ෂණය වේ. \n ඔවුන්ගේ කූඩු සාමාන්යයෙන් බිම හෝ ගස්වල පිහිටා ඇති අතර, ඔවුන් තම කූඩු ආක්රමණශීලී ලෙස ආරක්ෂා කරනු ඇත. ',
    imageUrl1: 'assets/wasp/blackjacket_2.jpg',
    imageUrl2: 'assets/wasp/blackjacket_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Dark Paper Wasp',
    scientificName: 'Polistes fuscatus',
    imageUrl: 'assets/wasp/dark_paper_wasp_1.jpg',
    venomousLevel: 'Venomous (not considered dangerous)',
    description:
        '- Dark Paper Wasps are a type of social wasp that are commonly found in North America. They are named for their dark coloration and the paper-like material that they use to build their nests. \n Dark Paper Wasps are known for their aggressive behavior, especially when their nests are disturbed.',
    appearance:
        '- Dark Paper Wasps are a medium-sized wasp, measuring about 1-2 inches in length. They have a black or dark brown body with yellow markings on their head and abdomen. \n Their wings are brownish-black in color. The shape of their body is slender with a narrow waist.',
    geographicLocation:
        '- Dark Paper Wasps are found throughout North America, from Canada to Mexico.',
    lifeAndHabits:
        '- Dark Paper Wasps are social wasps that live in colonies. The colony consists of a queen wasp and several female workers. \n The queen wasp lays eggs in the cells of the nest, and the female workers take care of the larvae and maintain the nest. Dark Paper Wasps are known for their aggressive behavior, and they will defend their nest vigorously if they feel threatened. They primarily feed on nectar and other insects, which they capture and feed to their larvae. \n Dark Paper Wasps are also known for their ability to learn and remember landmarks, which they use to navigate back to their nest.',
    sinhalaCommonName: 'කලාඳුරුවා',
    sinhalaScientificName: 'Polistes fuscatus',
    sinhalaVenomousLevel: 'විෂ සහිත (දරුණු ලෙස නොසැලකේ)',
    sinhalaDescription:
        '- Dark Paper Wasp උතුරු ඇමරිකාවේ බහුලව දක්නට ලැබෙන සමාජශීලී බඹර විශේෂයකි. ඔවුන්ගේ අඳුරු පැහැය සහ ඔවුන්ගේ කූඩු තැනීමට භාවිතා කරන කඩදාසි වැනි ද්රව්ය නිසා ඔවුන් මේ නමින් නම් කර ඇත. \n මොවුන් ආක්‍රමණශීලී හැසිරීම් සඳහා ප්‍රසිද්ධය, විශේෂයෙන් ඔවුන්ගේ කූඩුවලට බාධා ඇති විට.',
    sinhalaAppearance:
        '- Dark Paper Wasp මධ්‍යම ප්‍රමාණයේ බඹරෙකු වන අතර දිග අඟල් 1-2 ක් පමණ වේ. ඔවුන්ගේ හිසෙහි සහ උදරයේ කහ පැහැති සලකුණු සහිත කළු හෝ තද දුඹුරු පැහැති ශරීරයක් ඇත. \n ඔවුන්ගේ පියාපත් දුඹුරු-කළු වර්ණයෙන් යුක්ත වේ. ඔවුන්ගේ සිරුරේ හැඩය පටු ඉණක් සහිත සිහින් ය.',
    sinhalaGeographicLocation:
        '- මෙම බඹරුන් කැනඩාවේ සිට මෙක්සිකෝව දක්වා උතුරු ඇමරිකාව පුරා දක්නට ලැබේ.',
    sinhalaLifeAndHabits:
        '- මෙම බඹරුන් යනු ජනපදවල ජීවත් වන සමාජ බඹරුන් ය. ජනපදය සමන්විත වන්නේ බඹර රැජිනක් සහ කාන්තා සේවිකාවන් කිහිප දෙනෙකුගෙන් ය. බඹර රැජින කූඩුවේ සෛල තුළ බිත්තර දමයි, කාන්තා සේවිකාවන් කීටයන් රැකබලා ගනිමින් කූඩුව නඩත්තු කරයි. \n මෙම බඹරුන් ඔවුන්ගේ ආක්‍රමණශීලී හැසිරීම් සඳහා ප්‍රසිද්ධ වන අතර, ඔවුන්ට තර්ජනයක් දැනේ නම්, ඔවුන් තම කැදැල්ල දැඩි ලෙස ආරක්ෂා කරයි. ඔවුන් මූලික වශයෙන් මල් පැණි සහ අනෙකුත් කෘමීන් මත පෝෂණය වන අතර, ඔවුන් අල්ලා ඔවුන්ගේ කීටයන් පෝෂණය කරයි. \n අඳුරු කඩදාසි බඹරුන් ඔවුන්ගේ කූඩුව වෙත ආපසු යාමට භාවිතා කරන බිම් සලකුණු ඉගෙන ගැනීමට සහ මතක තබා ගැනීමට ඇති හැකියාව සඳහා ද ප්‍රසිද්ධය.',
    imageUrl1: 'assets/wasp/dark_paper_wasp_2.jpg',
    imageUrl2: 'assets/wasp/dark_paper_wasp_3.jpg',
  ),
  AnimalDetails(
    commonName: 'Yellowjacket',
    scientificName: 'Vespula or Dolichovespula',
    imageUrl: 'assets/wasp/yellowjacket_1.jpg',
    venomousLevel: 'Venomous (not considered dangerous)',
    description:
        '- Yellowjackets are a type of social wasp that are known for their yellow and black striped abdomen. They are typically between 10-16mm in length, and they have a slender waist and a pointed abdomen. \n Yellowjackets are known for their aggressive behavior, especially when their nests are disturbed.',
    appearance:
        '- Yellowjackets have black and yellow striped bodies, with the pattern varying slightly between species. They have a slender, elongated body with a narrow waist and a pointed abdomen. \n Their wings are translucent and folded lengthwise when at rest.',
    geographicLocation:
        '- Yellowjackets are found throughout the world, with species present in North America, Europe, Asia, and Australia.',
    lifeAndHabits:
        '- Yellowjackets are social wasps that live in colonies. The colony consists of a queen wasp and several female workers. The queen wasp lays eggs in the cells of the nest, and the female workers take care of the larvae and maintain the nest. \n Yellowjackets are known for their aggressive behavior, and they will defend their nest vigorously if they feel threatened. They primarily feed on nectar and other insects, which they capture and feed to their larvae. \n In late summer and early fall, the colony produces new queens and males, which mate and start new colonies the following year.',
    sinhalaCommonName: 'Yellowjacket',
    sinhalaScientificName: 'Vespula or Dolichovespula',
    sinhalaVenomousLevel: 'විෂ සහිත (දරුණු ලෙස නොසැලකේ)',
    sinhalaDescription:
        '- Yellowjacket යනු කහ සහ කළු ඉරි සහිත උදරයක් සහිත සමාජ බඹර වර්ගයකි. ඔවුන් සාමාන්‍යයෙන් දිග 10-16mm අතර වන අතර සිහින් ඉණක් සහ උල් වූ උදරයක් ඇත. විශේෂයෙන් ඔවුන්ගේ කූඩුවලට බාධා ඇති විට ඔවුන් ආක්‍රමණශීලී හැසිරීම් දක්වයි. ',
    sinhalaAppearance:
        '- මොවුන්ට කළු සහ කහ ඉරි සහිත සිරුරු ඇත, විශේෂයන් අතර රටාව තරමක් වෙනස් වේ. පටු ඉණක් සහ උල් වූ උදරයක් සහිත සිහින්, දිගටි සිරුරක් ඔවුන්ට ඇත. \n ඔවුන්ගේ පියාපත් පාරභාසක වන අතර විවේකයෙන් සිටින විට දිගට නැවී ඇත.',
    sinhalaGeographicLocation:
        '- මොවුන් ලොව පුරා දක්නට ලැබෙන අතර උතුරු ඇමරිකාව, යුරෝපය, ආසියාව සහ ඕස්ට්‍රේලියාව යන රටවල විශේෂ පවතී.',
    sinhalaLifeAndHabits:
        '- Yellowjackets යනු ජනපදවල ජීවත් වන සමාජශීලී බඹරුන් ය. ජනපදය සමන්විත වන්නේ බඹර රැජිනක් සහ කාන්තා සේවිකාවන් කිහිප දෙනෙකුගෙන් ය. බඹර රැජින කූඩුවේ සෛල තුළ බිත්තර දමයි, කාන්තා සේවිකාවන් කීටයන් රැකබලා ගනිමින් කූඩුව නඩත්තු කරයි. \n ඔවුන් ආක්‍රමණශීලී හැසිරීම් සඳහා ප්‍රසිද්ධ වී ඇති අතර, ඔවුන්ට තර්ජනයක් දැනෙනවා නම් ඔවුන් තම කැදැල්ල දැඩි ලෙස ආරක්ෂා කරයි. ඔවුන් මූලික වශයෙන් මල් පැණි සහ අනෙකුත් කෘමීන් මත පෝෂණය වන අතර, ඔවුන් අල්ලා ඔවුන්ගේ කීටයන් පෝෂණය කරයි. \n ග්‍රීෂ්ම සෘතුවේ අග සහ මුල් වැටීමේදී, ජනපදය නව බිසෝවරුන් සහ පිරිමින් බිහි කරයි, ඔවුන් සංසර්ගයේ යෙදී ඊළඟ වසරේ නව ජනපද ආරම්භ කරයි.',
    imageUrl1: 'assets/wasp/yellowjacket_2.jpg',
    imageUrl2: 'assets/wasp/yellowjacket_3.jpg',
  ),
];
